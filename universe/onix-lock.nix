...
} }:
{
  ...
  dune-configurator = {
    ...
    depends = with self; [ base-unix csexp dune ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  dune-rpc = {
    name = "dune-rpc";
  ...
  ipaddr = {
    ...
    depends = with self; [ domain-name dune macaddr ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  irmin = {
    name = "irmin";
  ...
  iter = {
    ...
    depends = with self; [ base-bytes dune dune-configurator ocaml result seq
  ...
  lwt = {
    ...
    depends = with self; [ dune dune-configurator ocaml ocplib-endian
                           (self.base-threads or null)
                           (self.base-unix or null) (self.conf-libev or null) ];
    buildDepends = with self; [ cppo dune dune-configurator ocaml ];
  };
  lwt_log = {
    name = "lwt_log";
    version = "1.1.2";
  ...
  mirage-crypto = {
    ...
    depends = with self; [ cstruct dune dune-configurator eqaf ocaml
                           (self.ocaml-freestanding or null) ];
    buildDepends = with self; [ conf-pkg-config dune dune-configurator ocaml ];
  };
  mirage-crypto-rng = {
  ...
  ocaml-lsp-server = {
    ...
    depends = with self; [ chrome-trace csexp dune dune-build-info dune-rpc
                           dyn fiber ocaml ocamlformat-rpc-lib octavius omd
                           ordering pp ppx_yojson_conv_lib re spawn stdune
                           uutf xdg yojson ];
    buildDepends = with self; [ dune ocaml ];
  };
  ocaml-syntax-shims = {
  ...
  odoc-parser = {
    ...
    depends = with self; [ astring camlp-streams dune ocaml result ];
    buildDepends = with self; [ dune ocaml ];
  };
  omd = {
    name = "omd";
  ...
  stdune = {
    ...
    depends = with self; [ base-unix csexp dune dyn ocaml ordering pp ];
    buildDepends = with self; [ dune ocaml ];
  };
  streaming = {
    name = "streaming";
