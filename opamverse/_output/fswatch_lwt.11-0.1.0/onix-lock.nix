...
} }:
{
  ...
  fswatch = {
    ...
    version = "11+0.1.0";
    src = pkgs.fetchurl {
      url = "https://github.com/kandu/ocaml-fswatch/archive/11-0.1.0.tar.gz";
      sha256 = "03nky24v4vgfr81i42z0q1pkx8xyjicjfcxg62b478dl143lx6gb";
    };
    opam = "${repo}/packages/fswatch/fswatch.11-0.1.0/opam";
    depends = with self; [ conf-fswatch dune ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  fswatch_lwt = {
    name = "fswatch_lwt";
    ...
    opam = "${repo}/packages/fswatch_lwt/fswatch_lwt.11-0.1.0/opam";
    depends = with self; [ dune fswatch lwt ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  lwt = {
    name = "lwt";
