...
} }:
{
  ...
  async_udp = {
    ...
    version = "v0.14.0";
    src = pkgs.fetchurl {
      url = "https://ocaml.janestreet.com/ocaml-core/v0.14/files/async_udp-v0.14.0.tar.gz";
      sha256 = "0lxfjz2kx1agdqdwkrcqi6cx8i9cizj97s735pzjn8p1liipks0c";
    };
    opam = "${repo}/packages/async_udp/async_udp.v0.14.0/opam";
    depends = with self; [ async core dune ocaml ppx_jane ];
    buildDepends = with self; [ dune ocaml ];
  };
  async_unix = {
    name = "async_unix";
  ...
  pixel_pusher = {
    ...
    version = "1.1";
    src = pkgs.fetchurl {
      url = "https://github.com/mbacarella/pixel_pusher/archive/1.1.tar.gz";
    ...
    opam = "${repo}/packages/pixel_pusher/pixel_pusher.1.1/opam";
    depends = with self; [ async_udp bitstring core dune ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  ppx_assert = {
    name = "ppx_assert";
