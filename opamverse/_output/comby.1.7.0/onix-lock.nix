...
} }:
{
  ...
  comby = {
    ...
    version = "1.7.0";
    src = pkgs.fetchurl {
      url = "https://github.com/ocaml/opam-source-archives/raw/main/comby-kernel.1.7.0.tar.gz";
    ...
    opam = "${repo}/packages/comby/comby.1.7.0/opam";
    depends = with self; [ cohttp-lwt-unix comby-kernel comby-semantic
                           conf-autoconf conf-gmp conf-libev conf-libpcre
                           conf-pkg-config conf-sqlite3 core dune lwt
                           lwt_react mparser mparser-pcre ocaml parany
                           patience_diff pcre ppx_deriving
                           ppx_deriving_yojson shell toml ];
    buildDepends = with self; [ dune ocaml ];
  };
  comby-kernel = {
    name = "comby-kernel";
    ...
    opam = "${repo}/packages/comby-kernel/comby-kernel.1.7.0/opam";
    depends = with self; [ bigstringaf core_kernel dune mparser mparser-pcre
                           ocaml pcre ppx_deriving ppx_deriving_yojson ];
    buildDepends = with self; [ dune ocaml ];
  };
  comby-semantic = {
    name = "comby-semantic";
    ...
    opam = "${repo}/packages/comby-semantic/comby-semantic.1.7.0/opam";
    depends = with self; [ cohttp cohttp-lwt-unix core_kernel dune lwt ocaml
                           ppx_deriving yojson ];
    buildDepends = with self; [ dune ocaml ];
  };
  conduit = {
  ...
  mparser-pcre = {
    ...
    opam = "${repo}/packages/mparser-pcre/mparser-pcre.1.3/opam";
    depends = with self; [ dune mparser ocaml pcre ];
    buildDepends = with self; [ dune ocaml ];
  };
  mtime = {
    name = "mtime";
  ...
  patience_diff = {
    ...
    version = "v0.14.0";
    src = pkgs.fetchurl {
      url = "https://ocaml.janestreet.com/ocaml-core/v0.14/files/patience_diff-v0.14.0.tar.gz";
      sha256 = "0hasiqwdc623mw2k39znslmcpnm9ynf4xkxzkfmx16dm0f4yj4gr";
    };
    opam = "${repo}/packages/patience_diff/patience_diff.v0.14.0/opam";
    depends = with self; [ base core_kernel dune ocaml ppx_jane ];
    buildDepends = with self; [ dune ocaml ];
  };
  pbkdf = {
    name = "pbkdf";
  ...
  shell = {
    ...
    version = "v0.14.0";
    src = pkgs.fetchurl {
      url = "https://ocaml.janestreet.com/ocaml-core/v0.14/files/shell-v0.14.0.tar.gz";
      sha256 = "0kzg38wf8pz0w8xyfhvj9ifcc0lkbsnlnfd0drmp7pgq8kypv96y";
    };
    opam = "${repo}/packages/shell/shell.v0.14.0/opam";
    depends = with self; [ core core_kernel dune jst-config ocaml ppx_jane
                           spawn textutils ];
    buildDepends = with self; [ dune ocaml ];
    depexts = with pkgs; [ (pkgs.linux-headers or null) ];
  };
