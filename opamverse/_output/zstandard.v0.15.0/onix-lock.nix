...
} }:
{
  ...
  conf-zstd = {
    name = "conf-zstd";
    version = "1.3.8";
    opam = "${repo}/packages/conf-zstd/conf-zstd.1.3.8/opam";
    buildDepends = with self; [ conf-pkg-config ];
    depexts = with pkgs; [ (pkgs.libzstd-dev or null)
                           (pkgs.libzstd-devel or null) (pkgs.zstd or null)
                           (pkgs.zstd-dev or null) ];
  };
  core = {
    name = "core";
    version = "v0.15.0";
  ...
  zstandard = {
    ...
    version = "v0.15.0";
    src = pkgs.fetchurl {
      url = "https://ocaml.janestreet.com/ocaml-core/v0.15/files/zstandard-v0.15.0.tar.gz";
      sha256 = "e1b9d4605d1c7825e1dfea4cdb4abe356f61acc32b14362535cd3c5d62352b36";
    };
    opam = "${repo}/packages/zstandard/zstandard.v0.15.0/opam";
    depends = with self; [ conf-zstd core core_kernel ctypes dune ocaml
                           ppx_jane ];
    buildDepends = with self; [ dune ocaml ];
  };
}
