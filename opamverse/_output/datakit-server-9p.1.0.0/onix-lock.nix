...
} }:
{
  ...
  datakit-server = {
    ...
    opam = "${repo}/packages/datakit-server/datakit-server.1.0.0/opam";
    depends = with self; [ astring cstruct dune fmt logs lwt ocaml rresult ];
    buildDepends = with self; [ dune ocaml ];
  };
  datakit-server-9p = {
    ...
    opam = "${repo}/packages/datakit-server-9p/datakit-server-9p.1.0.0/opam";
    depends = with self; [ datakit-server dune mirage-flow-lwt ocaml
                           protocol-9p sexplib ];
    buildDepends = with self; [ dune ocaml ];
  };
  dune = {
  ...
  protocol-9p = {
    ...
    version = "2.0.1";
    src = pkgs.fetchurl {
      url = "https://github.com/mirage/ocaml-9p/releases/download/v2.0.1/protocol-9p-v2.0.1.tbz";
      sha256 = "1r0s48wr5jh29hp4760afz69f1yh3bc35j62x6r7mq45m00kwx8z";
    };
    opam = "${repo}/packages/protocol-9p/protocol-9p.2.0.1/opam";
    depends = with self; [ astring base-bytes cstruct cstruct-sexp dune fmt
                           logs lwt mirage-channel-lwt mirage-flow-lwt ocaml
