...
} }:
{
  ...
  openai-gym = {
    ...
    version = "0.01";
    src = pkgs.fetchurl {
      url = "https://github.com/IBM/openai-gym-ocaml/archive/0.01.tar.gz";
      sha256 = "00ck25r4vcgs1zkv06n22hljca3nhhf7ida9z5qm2vz5na7v7fb1";
    };
    opam = "${repo}/packages/openai-gym/openai-gym.0.01/opam";
    depends = with self; [ atd cohttp-lwt-unix dune ocaml yojson ];
    buildDepends = with self; [ atdgen dune ocaml ocaml-migrate-parsetree ];
  };
  parsexp = {
    name = "parsexp";
    version = "v0.14.2";
