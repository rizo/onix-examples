...
} }:
{
  ...
  conf-wayland-protocols = {
    name = "conf-wayland-protocols";
    version = "1";
    opam = "${repo}/packages/conf-wayland-protocols/conf-wayland-protocols.1/opam";
    buildDepends = with self; [ conf-pkg-config ];
    depexts = with pkgs; [ (pkgs.wayland-protocols or null)
                           (pkgs.wayland-protocols-devel or null) ];
  };
  conf-xkbcommon = {
    name = "conf-xkbcommon";
    version = "1";
    ...
    buildDepends = with self; [ conf-pkg-config ];
    depexts = with pkgs; [ (pkgs.libxkbcommon or null)
                           (pkgs.libxkbcommon-dev or null)
  ...
  waylaunch = {
    ...
    version = "0.2.0";
    src = pkgs.fetchurl {
      url = "https://github.com/kit-ty-kate/waylaunch/releases/download/0.2.0/waylaunch-0.2.0.tbz";
      sha256 = "5adc39c3b57f7255d46f022edd263b00e8aadb9eec99dda1d092f27204ac641c";
    };
    opam = "${repo}/packages/waylaunch/waylaunch.0.2.0/opam";
    depends = with self; [ base-unix bos dune dune-configurator fpath ocaml
                           rresult ];
    buildDepends = with self; [ conf-cairo conf-libwayland conf-pango
                                conf-pkg-config conf-wayland-protocols
                                conf-xkbcommon dune ocaml ];
  };
}
