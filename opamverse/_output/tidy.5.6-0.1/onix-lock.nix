...
} }:
{
  ...
  conf-tidy = {
    name = "conf-tidy";
    version = "1";
    opam = "${repo}/packages/conf-tidy/conf-tidy.1/opam";
    depexts = with pkgs; [ (pkgs.app-text/tidy-html5 or null)
                           (pkgs.libtidy-dev or null)
                           (pkgs.libtidy-devel or null) (pkgs.tidy or null)
                           (pkgs.tidy-html5 or null) ];
  };
  core = {
    name = "core";
    version = "v0.15.0";
  ...
  tidy = {
    ...
    version = "5.6+0.1";
    src = pkgs.fetchurl {
      url = "https://github.com/kandu/ocaml-tidy/archive/5.6-0.1.tar.gz";
      sha256 = "1lq11aqxc6qwkjjc2xzb0ijczal1476b8zwy8dk38pfdnzsbwqcy";
    };
    opam = "${repo}/packages/tidy/tidy.5.6-0.1/opam";
    depends = with self; [ conf-tidy core_kernel dune ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  time_now = {
    name = "time_now";
