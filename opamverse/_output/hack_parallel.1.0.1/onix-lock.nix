...
} }:
{
  ...
  hack_parallel = {
    ...
    version = "1.0.1";
    src = pkgs.fetchurl {
      url = "https://github.com/rvantonder/hack_parallel/archive/1.0.1.tar.gz";
    ...
    opam = "${repo}/packages/hack_parallel/hack_parallel.1.0.1/opam";
    depends = with self; [ conf-pkg-config conf-sqlite3 core dune ocaml
                           ppx_deriving ppxlib sexplib ];
    buildDepends = with self; [ dune ocaml ];
  };
  jane-street-headers = {
