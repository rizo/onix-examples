...
} }:
{
  ...
  zbar = {
    ...
    opam = "${repo}/packages/zbar/zbar.0.9/opam";
    depends = with self; [ lwt ocaml (self.ctypes or null)
                           (self.ctypes-foreign or null) ];
    buildDepends = with self; [ oasis ocaml ocamlbuild ];
    depexts = with pkgs; [ (pkgs.libzbar-dev or null) ];
  };
}
