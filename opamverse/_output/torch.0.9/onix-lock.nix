...
} }:
{
  ...
  libtorch = {
    name = "libtorch";
    version = "1.5.0";
    opam = "${repo}/packages/libtorch/libtorch.1.5.0/opam";
    depexts = with pkgs; [ (pkgs.libomp or null) ];
  };
  npy = {
    name = "npy";
    version = "0.0.9";
  ...
  torch = {
    ...
    version = "0.9";
    src = pkgs.fetchurl {
      url = "https://github.com/LaurentMazare/ocaml-torch/archive/0.9b.tar.gz";
    ...
    opam = "${repo}/packages/torch/torch.0.9/opam";
    depends = with self; [ base cmdliner ctypes ctypes-foreign dune
                           dune-configurator libtorch npy ocaml
                           ocaml-compiler-libs ppx_custom_printf ppx_expect
                           ppx_sexp_conv sexplib stdio ];
    buildDepends = with self; [ dune ocaml ];
  };
}
