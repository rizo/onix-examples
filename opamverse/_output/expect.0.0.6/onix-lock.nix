...
} }:
{
  ...
  expect = {
    ...
    version = "0.0.6";
    src = pkgs.fetchurl {
      url = "https://download.ocamlcore.org/ocaml-expect/ocaml-expect/0.0.6/ocaml-expect-0.0.6.tar.gz";
      sha256 = "098qvg9d4yrqzr5ax291y3whrpax0m3sx4gi6is0mblc96r9yqk0";
    };
    opam = "${repo}/packages/expect/expect.0.0.6/opam";
    depends = with self; [ base-bytes batteries ocaml ocamlfind ounit pcre ];
    buildDepends = with self; [ ocaml ocamlbuild ocamlfind ];
  };
  num = {
    name = "num";
