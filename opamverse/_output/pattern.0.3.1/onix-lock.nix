...
} }:
{
  ...
  pattern = {
    ...
    version = "0.3.1";
    src = pkgs.fetchurl {
      url = "https://github.com/thierry-martinez/pattern/archive/v0.3.1.tar.gz";
    ...
    opam = "${repo}/packages/pattern/pattern.0.3.1/opam";
    depends = with self; [ dune metapp metaquot ocaml ocaml-migrate-parsetree
                           refl stdcompat ];
    buildDepends = with self; [ dune ocaml ];
  };
  ppx_derivers = {
