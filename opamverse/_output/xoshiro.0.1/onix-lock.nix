...
} }:
{
  ...
  make-random = {
    ...
    version = "0.1";
    src = pkgs.fetchurl {
      url = "https://github.com/LesBoloss-es/xoshiro/archive/0.1.tar.gz";
    ...
    opam = "${repo}/packages/make-random/make-random.0.1/opam";
  ...
  xoshiro = {
    ...
    opam = "${repo}/packages/xoshiro/xoshiro.0.1/opam";
    depends = with self; [ base-bigarray dune make-random ];
    buildDepends = with self; [ dune ];
  };
}
