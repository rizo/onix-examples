...
} }:
{
  ...
  spf = {
    ...
    opam = "${repo}/packages/spf/spf.2.0.2/opam";
    depends = with self; [ jbuilder ocaml ];
    buildDepends = with self; [ ocaml ];
    depexts = with pkgs; [ (pkgs.epel-release or null) (pkgs.libspf2 or null)
                           (pkgs.libspf2-dev or null)
                           (pkgs.libspf2-devel or null) ];
  };
}
