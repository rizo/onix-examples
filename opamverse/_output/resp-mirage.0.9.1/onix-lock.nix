...
} }:
{
  ...
  resp = {
    ...
    version = "0.9.1";
    src = pkgs.fetchurl {
      url = "https://github.com/zshipko/resp/archive/v0.9.1.tar.gz";
    ...
    opam = "${repo}/packages/resp/resp.0.9.1/opam";
  ...
  resp-client = {
    ...
    opam = "${repo}/packages/resp-client/resp-client.0.9.1/opam";
    depends = with self; [ dune ocaml resp ];
    buildDepends = with self; [ dune ocaml ];
  };
  resp-mirage = {
    name = "resp-mirage";
    ...
    opam = "${repo}/packages/resp-mirage/resp-mirage.0.9.1/opam";
    depends = with self; [ dune mirage-conduit ocaml resp resp-client
                           resp-server ];
    buildDepends = with self; [ dune ocaml ];
  };
  resp-server = {
    ...
    opam = "${repo}/packages/resp-server/resp-server.0.9.1/opam";
    depends = with self; [ conduit-lwt-unix dune ocaml resp resp-client ];
    buildDepends = with self; [ dune ocaml ];
  };
  result = {
    name = "result";
