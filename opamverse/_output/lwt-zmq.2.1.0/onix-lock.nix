...
} }:
{
  ...
  lwt-zmq = {
    ...
    opam = "${repo}/packages/lwt-zmq/lwt-zmq.2.1.0/opam";
    depends = with self; [ lwt ocaml zmq ];
  ...
  zmq = {
    ...
    opam = "${repo}/packages/zmq/zmq.4.0-8/opam";
    depends = with self; [ base-unix conf-zmq ocaml ocamlfind stdint ];
    buildDepends = with self; [ ocaml ocamlbuild ocamlfind ];
  };
}
