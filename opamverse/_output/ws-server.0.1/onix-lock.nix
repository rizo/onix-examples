...
} }:
{
  ...
  ws-server = {
    ...
    version = "0.1";
    src = pkgs.fetchurl {
      url = "https://github.com/roddyyaga/ws-server/archive/0.1.tar.gz";
    ...
    opam = "${repo}/packages/ws-server/ws-server.0.1/opam";
    depends = with self; [ dune lwt_ppx ocaml websocket-lwt-unix ];
    buildDepends = with self; [ dune ocaml ];
  };
  x509 = {
    name = "x509";
