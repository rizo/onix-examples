...
} }:
{
  ...
  kappa-agents = {
    ...
    version = "4.1.0";
    src = pkgs.fetchurl {
      url = "https://github.com/Kappa-Dev/KappaTools/archive/v4.1.tar.gz";
      sha256 = "8ca79f1917e3b5b0e3606547d8be374321f8eb68dfbe62e805339ac82bc89fd1";
    };
    opam = "${repo}/packages/kappa-agents/kappa-agents.4.1.0/opam";
    depends = with self; [ atdgen-runtime dune kappa-library ocaml ];
    buildDepends = with self; [ atdgen dune ocaml ];
  };
  kappa-binaries = {
    name = "kappa-binaries";
    version = "4.1.0";
    ...
    opam = "${repo}/packages/kappa-binaries/kappa-binaries.4.1.0/opam";
    depends = with self; [ dune kappa-library ocaml (self.labltk or null) ];
    buildDepends = with self; [ dune ocaml ];
  };
  kappa-library = {
    name = "kappa-library";
    ...
    opam = "${repo}/packages/kappa-library/kappa-library.4.1.0/opam";
    depends = with self; [ dune fmt logs lwt num ocaml re stdlib-shims yojson ];
    buildDepends = with self; [ dune ocaml ];
  };
  kappa-server = {
    name = "kappa-server";
    ...
    opam = "${repo}/packages/kappa-server/kappa-server.4.1.0/opam";
    depends = with self; [ cohttp-lwt-unix dune kappa-agents kappa-binaries
                           ocaml ];
    buildDepends = with self; [ atdgen dune ocaml ];
  };
  logs = {
