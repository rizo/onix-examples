...
} }:
{
  ...
  dns-resolver = {
    ...
    opam = "${repo}/packages/dns-resolver/dns-resolver.6.2.2/opam";
    depends = with self; [ dns dns-mirage dns-server dnssec dune duration lru
                           lwt mirage-clock mirage-random mirage-time ocaml
                           randomconv tcpip tls tls-mirage ];
    buildDepends = with self; [ dune ocaml ];
  };
  dns-server = {
    name = "dns-server";
  ...
  dns-stub = {
    ...
    opam = "${repo}/packages/dns-stub/dns-stub.6.2.2/opam";
    depends = with self; [ cstruct dns dns-client dns-mirage dns-resolver
                           dns-server dns-tsig dune duration lwt metrics
                           mirage-clock mirage-random mirage-time ocaml
                           randomconv tcpip ];
    buildDepends = with self; [ dune ocaml ];
  };
  dns-tsig = {
