...
} }:
{
  ...
  conformist = {
    ...
    version = "0.2.1";
    src = pkgs.fetchurl {
      url = "https://github.com/oxidizing/conformist/releases/download/0.2.1/conformist-0.2.1.tbz";
      sha256 = "80321b97c08f27985a678c64e24224267e420e5efa9f9c5c3a4a7da6aedc6ea8";
    };
    opam = "${repo}/packages/conformist/conformist.0.2.1/opam";
    depends = with self; [ dune ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  containers = {
  ...
  multipart-form-data = {
    ...
    opam = "${repo}/packages/multipart-form-data/multipart-form-data.0.3.0/opam";
    depends = with self; [ dune lwt lwt_ppx stringext ];
    buildDepends = with self; [ dune ];
  };
  num = {
    name = "num";
  ...
  opium = {
    ...
    opam = "${repo}/packages/opium/opium.0.20.0/opam";
    depends = with self; [ astring base64 cmdliner conf-libev dune fmt
                           httpaf-lwt-unix logs lwt magic-mime mirage-crypto
                           mtime multipart-form-data ocaml ptime re rock
                           tyxml uri yojson ];
    buildDepends = with self; [ dune ocaml ];
  };
  parsexp = {
  ...
  rock = {
    ...
    opam = "${repo}/packages/rock/rock.0.20.0/opam";
    depends = with self; [ bigstringaf dune hmap httpaf lwt ocaml sexplib0 ];
    buildDepends = with self; [ dune ocaml ];
  };
  safepass = {
    name = "safepass";
  ...
  sihl-contract = {
    ...
    version = "0.3.0+rc1";
    src = pkgs.fetchurl {
      url = "https://github.com/oxidizing/sihl/releases/download/0.3.0-rc1/sihl-0.3.0-rc1.tbz";
      sha256 = "ebfeaa62fe4498e6de03d7e6577e10c143d6148a10a6a47b98e032f3dcc70ddc";
    };
    opam = "${repo}/packages/sihl-contract/sihl-contract.0.3.0~rc1/opam";
    depends = with self; [ caqti-lwt dune ocaml opium sihl-core ];
    buildDepends = with self; [ dune ocaml ];
  };
  sihl-core = {
    name = "sihl-core";
    ...
    opam = "${repo}/packages/sihl-core/sihl-core.0.3.0~rc1/opam";
    depends = with self; [ caqti conformist dune fmt jwto logs lwt_ssl ocaml
                           ppx_deriving_yojson ppx_fields_conv ppx_sexp_conv
                           safepass sexplib ssl tls tsort uuidm yojson ];
    buildDepends = with self; [ dune ocaml ];
  };
  sihl-facade = {
    name = "sihl-facade";
    ...
    opam = "${repo}/packages/sihl-facade/sihl-facade.0.3.0~rc1/opam";
    depends = with self; [ dune ocaml sihl-contract sihl-core ];
    buildDepends = with self; [ dune ocaml ];
  };
  sihl-persistence = {
    ...
    opam = "${repo}/packages/sihl-persistence/sihl-persistence.0.3.0~rc1/opam";
    depends = with self; [ caqti caqti-lwt dune ocaml sihl-facade ];
    buildDepends = with self; [ dune ocaml ];
  };
  sihl-session = {
    name = "sihl-session";
    ...
    opam = "${repo}/packages/sihl-session/sihl-session.0.3.0~rc1/opam";
    depends = with self; [ dune ocaml sihl-facade sihl-persistence ];
    buildDepends = with self; [ dune ocaml ];
  };
  ssl = {
    name = "ssl";
  ...
  tsort = {
    ...
    version = "2.0.0";
    src = pkgs.fetchurl {
      url = "https://github.com/dmbaturin/ocaml-tsort/archive/2.0.0.zip";
    ...
    opam = "${repo}/packages/tsort/tsort.2.0.0/opam";
    depends = with self; [ containers dune ocaml ];
    buildDepends = with self; [ dune ocaml ];
