...
} }:
{
  ...
  cconv = {
    ...
    version = "0.5";
    src = pkgs.fetchurl {
      url = "https://github.com/c-cube/cconv/archive/0.5.tar.gz";
    ...
    opam = "${repo}/packages/cconv/cconv.0.5/opam";
    depends = with self; [ dune ocaml (self.bencode or null)
                           (self.sexplib or null) (self.yojson or null) ];
  ...
  toml-cconv = {
    ...
    opam = "${repo}/packages/toml-cconv/toml-cconv.5.0.0/opam";
    depends = with self; [ cconv dune ocaml toml ];
    buildDepends = with self; [ dune ocaml ];
  };
}
