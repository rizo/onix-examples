...
} }:
{
  ...
  rdf = {
    ...
    version = "0.13.0";
    src = pkgs.fetchurl {
      url = "https://framagit.org/zoggy/ocaml-rdf/-/archive/0.13.0/ocaml-rdf-0.13.0.tar.gz";
    ...
    opam = "${repo}/packages/rdf/rdf.0.13.0/opam";
    depends = with self; [ cryptokit dune dune-build-info fmt iri jsonm logs
                           menhir pcre ptime re sedlex uri uucp uuidm uutf
                           xmlm yojson ];
  ...
  rdf_mysql = {
    ...
    opam = "${repo}/packages/rdf_mysql/rdf_mysql.0.13.0/opam";
    depends = with self; [ dune mysql rdf ];
    buildDepends = with self; [ dune ];
  };
  re = {
    name = "re";
