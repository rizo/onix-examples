...
} }:
{
  ...
  cmdtui = {
    ...
    opam = "${repo}/packages/cmdtui/cmdtui.0.4.3/opam";
    depends = with self; [ astring jbuilder ocaml ];
    buildDepends = with self; [ ocaml ocamlfind ];
  };
  cmdtui-lambda-term = {
    ...
    opam = "${repo}/packages/cmdtui-lambda-term/cmdtui-lambda-term.0.4.3/opam";
    depends = with self; [ astring cmdliner cmdtui fmt jbuilder lambda-term
                           logs lwt ocaml ];
    buildDepends = with self; [ ocaml ];
  };
  cppo = {
  ...
  lambda-term = {
    ...
    version = "1.13";
    src = pkgs.fetchurl {
      url = "https://github.com/ocaml-community/lambda-term/releases/download/1.13/lambda-term-1.13.tbz";
      sha256 = "000jw2pfkh25flqrbpggpk67x9q3j5cgfrlaf4f4djajrcvpxwmd";
    };
    opam = "${repo}/packages/lambda-term/lambda-term.1.13/opam";
    depends = with self; [ camomile jbuilder lwt lwt_log lwt_react ocaml
                           react zed ];
    buildDepends = with self; [ ocaml ];
  };
  logs = {
  ...
  zed = {
    ...
    version = "1.6";
    src = pkgs.fetchurl {
      url = "https://github.com/ocaml-community/zed/releases/download/1.6/zed-1.6.tbz";
      sha256 = "07hyiywffpmfks23lffqw0csyi6r51igwb2qg4hd4nwzxvr4ifvl";
    };
    opam = "${repo}/packages/zed/zed.1.6/opam";
    depends = with self; [ base-bytes camomile jbuilder ocaml react ];
    buildDepends = with self; [ ocaml ];
  };
}
