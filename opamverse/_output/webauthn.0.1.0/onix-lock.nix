...
} }:
{
  ...
  cbor = {
    ...
    version = "0.5";
    src = pkgs.fetchurl {
      url = "https://ygrek.org/p/release/ocaml-cbor/ocaml-cbor-0.5.tar.gz";
      sha256 = "e26a66fdfbfd5f9b854503bc5ea0613a9c58c19ade12d2777b123037a62e94a3";
    };
    opam = "${repo}/packages/cbor/cbor.0.5/opam";
    depends = with self; [ base-bytes dune ocaml ocplib-endian ];
    buildDepends = with self; [ dune ocaml ];
  };
  conf-gmp = {
    name = "conf-gmp";
  ...
  webauthn = {
    ...
    version = "0.1.0";
    src = pkgs.fetchurl {
      url = "https://github.com/roburio/webauthn/releases/download/v0.1.0/webauthn-0.1.0.tbz";
      sha256 = "c9082576c49d34f49884c361afb5f856b82ff7066d1d6f3b3d03c41314449701";
    };
    opam = "${repo}/packages/webauthn/webauthn.0.1.0/opam";
    depends = with self; [ base64 cbor cstruct dune mirage-crypto-ec
                           mirage-crypto-rng ocaml ocplib-endian
                           ppx_deriving_yojson x509 yojson ];
    buildDepends = with self; [ dune ocaml ];
  };
  x509 = {
    name = "x509";
