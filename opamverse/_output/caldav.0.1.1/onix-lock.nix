...
} }:
{
  ...
  caldav = {
    ...
    version = "0.1.1";
    src = pkgs.fetchurl {
      url = "https://github.com/roburio/caldav/releases/download/v0.1.1/caldav-v0.1.1.tbz";
      sha256 = "edcd9e1b65b7595e5dde49c1b2aac9cf25c60cd713eb25cbdb80df8fbfb5b80a";
    };
    opam = "${repo}/packages/caldav/caldav.0.1.1/opam";
    depends = with self; [ base64 cohttp cohttp-lwt dispatch dune fmt hex
                           icalendar logs lwt metrics mirage-clock
                           mirage-crypto mirage-crypto-rng mirage-kv
                           mirage-random ocaml ppx_deriving ppx_sexp_conv
                           ptime re sexplib tyxml uri xmlm ];
    buildDepends = with self; [ dune ocaml ];
  };
  cohttp = {
    name = "cohttp";
