...
} }:
{
  ...
  lambda-runtime = {
    ...
    opam = "${repo}/packages/lambda-runtime/lambda-runtime.0.1.0/opam";
    depends = with self; [ dune logs lwt ocaml piaf ppx_deriving_yojson
                           result uri yojson ];
    buildDepends = with self; [ dune ocaml ];
  };
  logs = {
  ...
  mrmime = {
    ...
    opam = "${repo}/packages/mrmime/mrmime.0.3.2/opam";
    depends = with self; [ angstrom base64 bigarray-compat bigarray-overlap
                           bigstringaf dune emile fmt ipaddr ke ocaml pecu
                           ptime rosetta rresult unstrctrd uutf ];
    buildDepends = with self; [ dune ocaml ];
  };
  ocaml = {
    name = "ocaml";
  ...
  piaf = {
    ...
    opam = "${repo}/packages/piaf/piaf.0.1.0/opam";
    depends = with self; [ angstrom bigstringaf conf-libssl dune faraday
                           gluten-lwt-unix logs lwt lwt_ssl magic-mime mrmime
                           ocaml pecu psq ssl stdlib-shims uri ];
    buildDepends = with self; [ dune ocaml ];
  };
  ppx_derivers = {
    name = "ppx_derivers";
  ...
  vercel = {
    ...
    opam = "${repo}/packages/vercel/vercel.0.1.0/opam";
    depends = with self; [ base64 dune lambda-runtime lwt ocaml piaf
