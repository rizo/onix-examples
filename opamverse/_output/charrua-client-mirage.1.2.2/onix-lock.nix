...
} }:
{
  ...
  charrua-client = {
    ...
    opam = "${repo}/packages/charrua-client/charrua-client.1.2.2/opam";
    depends = with self; [ charrua cstruct dune ipaddr macaddr ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  charrua-client-lwt = {
    ...
    opam = "${repo}/packages/charrua-client-lwt/charrua-client-lwt.1.2.2/opam";
    depends = with self; [ charrua charrua-client cstruct dune duration fmt
                           ipaddr logs lwt mirage-net mirage-random
                           mirage-time ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  charrua-client-mirage = {
    ...
    opam = "${repo}/packages/charrua-client-mirage/charrua-client-mirage.1.2.2/opam";
    depends = with self; [ charrua-client-lwt dune ipaddr logs lwt
                           mirage-clock mirage-net mirage-protocols
                           mirage-random mirage-time ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  cppo = {
    name = "cppo";
  ...
  mirage-device = {
    ...
    opam = "${repo}/packages/mirage-device/mirage-device.2.0.0/opam";
  ...
  mirage-protocols = {
    ...
    version = "4.0.1";
    src = pkgs.fetchurl {
      url = "https://github.com/mirage/mirage-protocols/releases/download/v4.0.1/mirage-protocols-v4.0.1.tbz";
      sha256 = "b10e3a8f530d48e424ba089fe9e404c2e3b6f0420ea99b39a52bf0d64d4715a1";
    };
    opam = "${repo}/packages/mirage-protocols/mirage-protocols.4.0.1/opam";
    depends = with self; [ dune duration fmt ipaddr lwt macaddr mirage-device
                           mirage-flow ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  mirage-random = {
  ...
  tcpip = {
    ...
    version = "5.0.1";
    src = pkgs.fetchurl {
      url = "https://github.com/mirage/mirage-tcpip/releases/download/v5.0.1/tcpip-v5.0.1.tbz";
      sha256 = "bc9b7238b5a8baa4d4c6a2210b85106af3193988c8229a40eaad70c6f6b4bf35";
    };
    opam = "${repo}/packages/tcpip/tcpip.5.0.1/opam";
    depends = with self; [ cstruct cstruct-lwt dune dune-configurator
                           duration ethernet fmt ipaddr logs lru lwt
                           lwt-dllist macaddr macaddr-cstruct mirage-clock
                           mirage-net mirage-profile mirage-protocols
                           mirage-random mirage-stack mirage-time ocaml
                           randomconv rresult (self.mirage-xen-ocaml or null) ];
    buildDepends = with self; [ dune ocaml ];
  };
  topkg = {
    name = "topkg";
