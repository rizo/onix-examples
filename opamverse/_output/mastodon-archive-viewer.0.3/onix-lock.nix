...
} }:
{
  ...
  ffmpeg = {
    ...
    opam = "${repo}/packages/ffmpeg/ffmpeg.0.4.3/opam";
    depends = with self; [ base-bigarray base-threads ocaml ];
  ...
  mastodon-archive-viewer = {
    ...
    version = "0.3";
    src = pkgs.fetchurl {
      url = "https://github.com/kit-ty-kate/mastodon-archive-viewer/archive/0.3.tar.gz";
      sha256 = "0xv8im56pzdadrlmh7h3m16n663y262kyhay6avj4286lwpna19b";
    };
    opam = "${repo}/packages/mastodon-archive-viewer/mastodon-archive-viewer.0.3/opam";
    depends = with self; [ cmdliner containers dune ezjsonm ffmpeg ocaml
                           ptime tyxml ];
    buildDepends = with self; [ dune ocaml ];
  };
  ocaml = {
