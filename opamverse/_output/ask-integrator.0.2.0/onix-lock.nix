...
} }:
{
  ...
  ask = {
    ...
    version = "0.2.0";
    src = pkgs.fetchurl {
      url = "https://github.com/uzh/ask/archive/0.2.0.tar.gz";
    ...
    opam = "${repo}/packages/ask/ask.0.2.0/opam";
    depends = with self; [ conformist dune lwt_ssl ocaml ppx_fields_conv sihl
                           sihl-storage ];
  ...
  ask-integrator = {
    ...
    opam = "${repo}/packages/ask-integrator/ask-integrator.0.2.0/opam";
    depends = with self; [ ask conformist dune lwt_ssl ocaml ppx_fields_conv
                           sihl sihl-storage ];
    buildDepends = with self; [ dune ocaml ];
  };
  asn1-combinators = {
  ...
  conformist = {
    ...
    version = "0.4.0";
    src = pkgs.fetchurl {
      url = "https://github.com/oxidizing/conformist/releases/download/0.4.0/conformist-0.4.0.tbz";
      sha256 = "cabb3e2c2c828d264703a9e42e0a47ee9d766ce129bca4c20eb511d812622623";
    };
    opam = "${repo}/packages/conformist/conformist.0.4.0/opam";
    depends = with self; [ dune ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  containers = {
  ...
  sihl = {
    ...
    version = "0.4.0";
    src = pkgs.fetchurl {
      url = "https://github.com/oxidizing/sihl/archive/0.4.0.tar.gz";
    ...
    opam = "${repo}/packages/sihl/sihl.0.4.0/opam";
    depends = with self; [ caqti caqti-lwt conformist containers dune fmt
                           jwto logs lwt_ssl ocaml opium ppx_deriving_yojson
                           ppx_fields_conv ppx_sexp_conv safepass sexplib ssl
                           tls tsort uuidm yojson ];
    buildDepends = with self; [ dune ocaml ];
  };
  sihl-storage = {
    name = "sihl-storage";
    ...
    opam = "${repo}/packages/sihl-storage/sihl-storage.0.4.0/opam";
    depends = with self; [ dune ocaml sihl ];
    buildDepends = with self; [ dune ocaml ];
  };
  ssl = {
    name = "ssl";
