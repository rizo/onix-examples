...
} }:
{
  ...
  rpc = {
    ...
    opam = "${repo}/packages/rpc/rpc.8.1.2/opam";
    depends = with self; [ dune ocaml ppx_deriving_rpc rpclib rpclib-lwt ];
    buildDepends = with self; [ dune ocaml ];
  };
  rpclib = {
    name = "rpclib";
  ...
  rpclib-lwt = {
    ...
    opam = "${repo}/packages/rpclib-lwt/rpclib-lwt.8.1.2/opam";
    depends = with self; [ dune lwt ocaml rpclib ];
    buildDepends = with self; [ dune ocaml ];
  };
  rresult = {
    name = "rresult";
  ...
  vhdlib = {
    ...
    opam = "${repo}/packages/vhdlib/vhdlib.0.9.1/opam";
    depends = with self; [ ocaml ocamlfind rpc ];
    buildDepends = with self; [ ocaml ocamlbuild ocamlfind ];
    depexts = with pkgs; [ (pkgs.blktap-dev or null)
                           (pkgs.blktap-devel or null)
                           (pkgs.libuuid-devel or null)
                           (pkgs.uuid-dev or null) ];
  };
  xmlm = {
    name = "xmlm";
    version = "1.4.0";
