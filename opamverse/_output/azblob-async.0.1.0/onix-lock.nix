...
} }:
{
  ...
  azblob = {
    ...
    version = "0.1.0";
    src = pkgs.fetchurl {
      url = "https://github.com/kkazuo/azblob/archive/0.1.0.tar.gz";
    ...
    opam = "${repo}/packages/azblob/azblob.0.1.0/opam";
    depends = with self; [ base base64 cohttp cryptokit dune ocaml
                           ppx_sexp_conv sexplib0 uri ];
    buildDepends = with self; [ dune ocaml ];
  };
  azblob-async = {
    name = "azblob-async";
    ...
    opam = "${repo}/packages/azblob-async/azblob-async.0.1.0/opam";
    depends = with self; [ azblob cohttp cohttp-async core dune ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  base = {
    name = "base";
