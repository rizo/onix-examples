...
} }:
{
  ...
  conf-mecab = {
    name = "conf-mecab";
    version = "0.996";
    opam = "${repo}/packages/conf-mecab/conf-mecab.0.996/opam";
    depexts = with pkgs; [ (pkgs.libmecab-dev or null) (pkgs.mecab or null)
                           (pkgs.mecab-devel or null) ];
  };
  csexp = {
    name = "csexp";
    version = "1.5.1";
  ...
  mecab = {
    ...
    opam = "${repo}/packages/mecab/mecab.0.0.0/opam";
    depends = with self; [ camomile conf-mecab jbuilder ocaml ppx_sexp_conv
                           sexplib ];
    buildDepends = with self; [ ocaml ocaml-migrate-parsetree ocamlfind ];
  };
  num = {
    name = "num";
    version = "1.4";
