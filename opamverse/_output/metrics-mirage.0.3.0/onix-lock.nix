...
} }:
{
  ...
  metrics = {
    ...
    opam = "${repo}/packages/metrics/metrics.0.3.0/opam";
    depends = with self; [ dune fmt ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  metrics-influx = {
    ...
    opam = "${repo}/packages/metrics-influx/metrics-influx.0.3.0/opam";
    depends = with self; [ astring dune duration fmt lwt metrics ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  metrics-mirage = {
    name = "metrics-mirage";
    ...
    opam = "${repo}/packages/metrics-mirage/metrics-mirage.0.3.0/opam";
    depends = with self; [ cstruct dune ipaddr logs lwt metrics
                           metrics-influx mirage-clock mirage-stack ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  mirage-clock = {
    name = "mirage-clock";
