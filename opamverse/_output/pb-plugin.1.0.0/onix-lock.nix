...
} }:
{
  ...
  pb = {
    ...
    version = "1.0.0";
    src = pkgs.fetchurl {
      url = "https://github.com/yallop/ocaml-pb/releases/download/1.0.0/pb-1.0.0.tbz";
      sha256 = "e670f11879ee5e44de396fda26ce18384093fd9d75b33727fe90db073e1e4d01";
    };
    opam = "${repo}/packages/pb/pb.1.0.0/opam";
    depends = with self; [ angstrom dune faraday integers ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  pb-plugin = {
    name = "pb-plugin";
    ...
    opam = "${repo}/packages/pb-plugin/pb-plugin.1.0.0/opam";
    depends = with self; [ angstrom batteries dune faraday integers ocaml pb ];
    buildDepends = with self; [ dune ocaml ];
  };
  result = {
    name = "result";
