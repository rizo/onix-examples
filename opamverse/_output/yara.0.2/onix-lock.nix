...
} }:
{
  ...
  yara = {
    ...
    version = "0.2";
    src = pkgs.fetchurl {
      url = "https://github.com/XVilka/yara-ocaml/archive/0.2.tar.gz";
    ...
    opam = "${repo}/packages/yara/yara.0.2/opam";
    depends = with self; [ core ctypes ctypes-foreign dune ocaml ppx_deriving ];
    buildDepends = with self; [ dune ocaml ocaml-migrate-parsetree ];
    depexts = with pkgs; [ (pkgs.epel-release or null)
                           (pkgs.libyara-dev or null)
                           (pkgs.libyara-devel or null) (pkgs.yara or null)
                           (pkgs.yara-devel or null) ];
  };
}
