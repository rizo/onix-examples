...
} }:
{
  ...
  pgx_async = {
    ...
    opam = "${repo}/packages/pgx_async/pgx_async.2.0/opam";
    depends = with self; [ async_kernel async_ssl async_unix conduit-async
                           dune ocaml pgx pgx_value_core ];
    buildDepends = with self; [ dune ocaml ];
  };
  pgx_value_core = {
    name = "pgx_value_core";
    ...
    opam = "${repo}/packages/pgx_value_core/pgx_value_core.2.0/opam";
    depends = with self; [ core_kernel dune ocaml pgx ];
    buildDepends = with self; [ dune ocaml ];
  };
  ppx_assert = {
    name = "ppx_assert";
