...
} }:
{
  ...
  github-hooks = {
    ...
    version = "0.5.0";
    src = pkgs.fetchurl {
      url = "https://github.com/dsheets/ocaml-github-hooks/releases/download/0.5.0/github-hooks-0.5.0.tbz";
      sha256 = "bd92e6d3b8f6106e65851ebb0e8d56f8f87dd2d7cf09ecaae2ea5f5ed422f4c9";
    };
    opam = "${repo}/packages/github-hooks/github-hooks.0.5.0/opam";
    depends = with self; [ cohttp-lwt conduit-lwt cstruct dune fmt github hex
                           logs lwt nocrypto ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  github-hooks-unix = {
    ...
    opam = "${repo}/packages/github-hooks-unix/github-hooks-unix.0.5.0/opam";
    depends = with self; [ cohttp-lwt-unix conduit-lwt-unix dune github-hooks
                           github-unix ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  github-unix = {
