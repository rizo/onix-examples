...
} }:
{
  ...
  ssh-agent = {
    ...
    version = "0.4.0";
    src = pkgs.fetchurl {
      url = "https://github.com/reynir/ocaml-ssh-agent/releases/download/v0.4.0/ssh-agent-0.4.0.tbz";
      sha256 = "f3e931d8c7eeac926cf353079d653a2a2757e238af3a45555d5cab1d8460a956";
    };
    opam = "${repo}/packages/ssh-agent/ssh-agent.0.4.0/opam";
    depends = with self; [ angstrom cstruct dune faraday mirage-crypto
  ...
  ssh-agent-unix = {
    ...
    opam = "${repo}/packages/ssh-agent-unix/ssh-agent-unix.0.4.0/opam";
    depends = with self; [ angstrom-unix dune faraday ocaml ssh-agent ];
    buildDepends = with self; [ dune ocaml ];
  };
  stdlib-shims = {
    name = "stdlib-shims";
