...
} }:
{
  ...
  hyper = {
    ...
    version = "1.0.0+alpha1";
    src = pkgs.fetchurl {
      url = "https://github.com/aantron/hyper/archive/refs/tags/1.0.0-alpha1.tar.gz";
      sha256 = "14hiszal3sp0rck6qhvs5f8d4d2bb09ay4sl1kgvap4qppirmav4";
    };
    opam = "${repo}/packages/hyper/hyper.1.0.0~alpha1/opam";
    depends = with self; [ dream-httpaf dream-pure dune lwt_ppx
                           mirage-crypto-rng ocaml uri ];
    buildDepends = with self; [ dune ocaml ];
  };
  logs = {
    name = "logs";
