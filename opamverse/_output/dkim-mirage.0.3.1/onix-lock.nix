...
} }:
{
  ...
  dkim = {
    ...
    version = "0.3.1";
    src = pkgs.fetchurl {
      url = "https://github.com/dinosaure/ocaml-dkim/releases/download/v0.3.1/dkim-0.3.1.tbz";
      sha256 = "bf428053e08e8c438686abe0fc3c59ceb74ab353a0dbae55d58e3bdcccc57db0";
    };
    opam = "${repo}/packages/dkim/dkim.0.3.1/opam";
    depends = with self; [ astring base-unix base64 cmdliner digestif
                           dns-client domain-name dune fmt fpath hmap ipaddr
                           logs mirage-crypto mirage-crypto-pk mrmime ocaml
                           x509 ];
    buildDepends = with self; [ dune ocaml ];
  };
  dkim-mirage = {
    ...
    opam = "${repo}/packages/dkim-mirage/dkim-mirage.0.3.1/opam";
    depends = with self; [ dkim dns-client dune lwt mirage-clock
