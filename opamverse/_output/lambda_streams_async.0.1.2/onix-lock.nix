...
} }:
{
  ...
  lambda_streams = {
    ...
    version = "0.1.2";
    src = pkgs.fetchurl {
      url = "https://github.com/Risto-Stevcev/lambda-streams/archive/0.1.2.tar.gz";
    ...
    opam = "${repo}/packages/lambda_streams/lambda_streams.0.1.2/opam";
    depends = with self; [ dune ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  lambda_streams_async = {
    ...
    opam = "${repo}/packages/lambda_streams_async/lambda_streams_async.0.1.2/opam";
    depends = with self; [ async async_kernel async_unix core dune
                           lambda_streams ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  num = {
    name = "num";
