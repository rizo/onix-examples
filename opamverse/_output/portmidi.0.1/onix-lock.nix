...
} }:
{
  ...
  conf-libportmidi = {
    name = "conf-libportmidi";
    version = "1";
    opam = "${repo}/packages/conf-libportmidi/conf-libportmidi.1/opam";
    buildDepends = with self; [ conf-pkg-config ];
    depexts = with pkgs; [ (pkgs.libportmidi-dev or null)
                           (pkgs.portmidi or null)
                           (pkgs.portmidi-dev or null)
                           (pkgs.portmidi-devel or null) ];
  };
  conf-pkg-config = {
  ...
  portmidi = {
    ...
    version = "0.1";
    src = pkgs.fetchurl {
      url = "https://github.com/mbacarella/portmidi/archive/0.1.tar.gz";
    ...
    opam = "${repo}/packages/portmidi/portmidi.0.1/opam";
    depends = with self; [ conf-libportmidi conf-pkg-config core_kernel
                           ctypes ctypes-foreign dune dune-configurator ocaml
                           ppx_jane ];
    buildDepends = with self; [ dune ocaml ];
  };
  ppx_assert = {
