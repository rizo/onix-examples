...
} }:
{
  ...
  conex = {
    ...
    opam = "${repo}/packages/conex/conex.0.9.2/opam";
    depends = with self; [ astring cmdliner cstruct fmt logs nocrypto ocaml
                           opam-file-format rresult x509
                           (self.opam-format or null) ];
    buildDepends = with self; [ ocaml ocamlbuild ocamlfind topkg ];
  };
  conf-gmp = {
    name = "conf-gmp";
