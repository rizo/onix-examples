...
} }:
{
  ...
  angstrom-async = {
    ...
    opam = "${repo}/packages/angstrom-async/angstrom-async.0.15.0/opam";
    depends = with self; [ angstrom async dune ocaml ];
  ...
  vcaml = {
    ...
    version = "v0.15.0";
    src = pkgs.fetchurl {
      url = "https://ocaml.janestreet.com/ocaml-core/v0.15/files/vcaml-v0.15.0.tar.gz";
      sha256 = "0dbf2526a24d838988ae9a327550fdd9f0328dbdca9d026430fb4b579e0e0442";
    };
    opam = "${repo}/packages/vcaml/vcaml.v0.15.0/opam";
    depends = with self; [ angstrom angstrom-async async base core
                           core_kernel core_unix dune
                           expect_test_helpers_async faraday ocaml ppx_jane
                           textutils ];
    buildDepends = with self; [ dune ocaml ];
  };
}
