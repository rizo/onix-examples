...
} }:
{
  ...
  jhupllib = {
    ...
    version = "0.2.2";
    src = pkgs.fetchurl {
      url = "https://github.com/JHU-PL-Lab/jhupllib/archive/0.2.2.tar.gz";
    ...
    opam = "${repo}/packages/jhupllib/jhupllib.0.2.2/opam";
    depends = with self; [ base-threads batteries dune monadlib ocaml
                           ocaml-monadic ppx_deriving ppx_deriving_yojson
                           yojson ];
    buildDepends = with self; [ dune ocaml ];
  };
  monadlib = {
    ...
    opam = "${repo}/packages/monadlib/monadlib.0.2/opam";
    depends = with self; [ batteries oasis ocaml ocamlfind ];
    buildDepends = with self; [ ocaml ocamlbuild ocamlfind ];
  };
  num = {
    name = "num";
  ...
  pds-reachability = {
    ...
    version = "0.2.2";
    src = pkgs.fetchurl {
      url = "https://github.com/JHU-PL-Lab/pds-reachability/archive/b425d6f83d811dfa4c70e96d3bead4dfa257169e.zip";
    ...
    opam = "${repo}/packages/pds-reachability/pds-reachability.0.2.2/opam";
    depends = with self; [ base-threads batteries jbuilder jhupllib ocaml
                           ocaml-monadic ppx_deriving ppx_deriving_yojson
                           yojson ];
    buildDepends = with self; [ ocaml ocamlbuild ocamlfind ounit ];
    depexts = with pkgs; [ unzip ];
  };
  ppx_derivers = {
    name = "ppx_derivers";
