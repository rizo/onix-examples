...
} }:
{
  ...
  session-postgresql = {
    ...
    opam = "${repo}/packages/session-postgresql/session-postgresql.0.5.0/opam";
    depends = with self; [ dune ocaml postgresql session ];
    buildDepends = with self; [ dune ocaml ];
  };
  session-postgresql-async = {
    ...
    opam = "${repo}/packages/session-postgresql-async/session-postgresql-async.0.5.0/opam";
    depends = with self; [ async base-threads dune ocaml session-postgresql ];
    buildDepends = with self; [ dune ocaml ];
  };
  sexp_pretty = {
    name = "sexp_pretty";
