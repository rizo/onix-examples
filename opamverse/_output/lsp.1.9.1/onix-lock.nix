...
} }:
{
  ...
  jsonrpc = {
    ...
    version = "1.9.1";
    src = pkgs.fetchurl {
      url = "https://github.com/ocaml/ocaml-lsp/releases/download/1.9.1/jsonrpc-1.9.1.tbz";
      sha256 = "cf1cdb257d4ef1ed1a05e951264b11c7dc7d3066bf448111cb17c67be56ddcee";
    };
    opam = "${repo}/packages/jsonrpc/jsonrpc.1.9.1/opam";
    depends = with self; [ dune ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  lsp = {
    ...
    opam = "${repo}/packages/lsp/lsp.1.9.1/opam";
    depends = with self; [ csexp dune jsonrpc ocaml pp ppx_yojson_conv_lib
                           result uutf yojson ];
    buildDepends = with self; [ dune ocaml ];
  };
  ocaml = {
