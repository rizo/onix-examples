...
} }:
{
  ...
  omigrate = {
    ...
    version = "0.1.1";
    src = pkgs.fetchurl {
      url = "https://github.com/tmattio/omigrate/releases/download/0.1.1/omigrate-0.1.1.tbz";
      sha256 = "274a85d581eaccdc291a1cfa7266c8e634e65f32e1c5ac54be3094d3d64ebad2";
    };
    opam = "${repo}/packages/omigrate/omigrate.0.1.1/opam";
    depends = with self; [ cmdliner dune fmt logs lwt ocaml pgx pgx_lwt_unix
                           uri ];
    buildDepends = with self; [ dune ocaml ];
  };
  pgx = {
  ...
  pgx_lwt_unix = {
    ...
    opam = "${repo}/packages/pgx_lwt_unix/pgx_lwt_unix.2.0/opam";
    depends = with self; [ dune ocaml pgx pgx_lwt ];
    buildDepends = with self; [ dune ocaml ];
  };
  ppx_compare = {
    name = "ppx_compare";
