...
} }:
{
  ...
  mirage-vnetif = {
    ...
    version = "0.6.0";
    src = pkgs.fetchurl {
      url = "https://github.com/mirage/mirage-vnetif/releases/download/v0.6.0/mirage-vnetif-0.6.0.tbz";
      sha256 = "7f3468345a9d9e3e0a7be78d768e5caef6e7283c7affe750caef8adeb0f9758c";
    };
    opam = "${repo}/packages/mirage-vnetif/mirage-vnetif.0.6.0/opam";
    depends = with self; [ cstruct dune duration ipaddr logs lwt macaddr
                           mirage-net mirage-profile ocaml result ];
    buildDepends = with self; [ dune ocaml ];
  };
  mirage-vnetif-stack = {
    ...
    opam = "${repo}/packages/mirage-vnetif-stack/mirage-vnetif-stack.0.6.0/opam";
    depends = with self; [ arp cstruct dune duration ethernet ipaddr logs lwt
                           macaddr mirage-clock mirage-net mirage-profile
                           mirage-random mirage-time mirage-vnetif ocaml
                           result tcpip ];
    buildDepends = with self; [ dune ocaml ];
  };
  mmap = {
