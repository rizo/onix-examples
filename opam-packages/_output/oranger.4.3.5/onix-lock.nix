...
} }:
{
  ...
  oranger = {
    ...
    version = "4.3.5";
    src = pkgs.fetchurl {
      url = "https://github.com/UnixJunkie/oranger/archive/v4.3.5.tar.gz";
      sha256 = "0d9v7a6shcbf46n23s51y4znvfyfc03agvqnnw66nay4y6kdj5dv";
    };
    opam = "${repo}/packages/oranger/oranger.4.3.5/opam";
    depends = with self; [ batteries cpm dolog dune line_oriented minicli
                           molenc parany re (self.conf-gnuplot or null) ];
    buildDepends = with self; [ conf-cmake dune ];
  };
  parany = {
    name = "parany";
    version = "12.2.0";
