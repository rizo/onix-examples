...
} }:
{
  ...
  conf-libsodium = {
    name = "conf-libsodium";
    version = "1";
    opam = "${repo}/packages/conf-libsodium/conf-libsodium.1/opam";
    buildDepends = with self; [ conf-pkg-config ];
    depexts = with pkgs; [ (pkgs.libsodium or null)
                           (pkgs.libsodium-dev or null)
                           (pkgs.security/libsodium or null) ];
  };
  conf-pkg-config = {
  ...
  sodium = {
    ...
    opam = "${repo}/packages/sodium/sodium.0.6.0/opam";
    depends = with self; [ base-bigarray base-bytes conf-libsodium ctypes
                           ocaml ];
