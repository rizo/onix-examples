...
} }:
{
  albatross = {
    ...
    version = "1.5.0";
    src = pkgs.fetchurl {
      url = "https://github.com/roburio/albatross/releases/download/v1.5.0/albatross-1.5.0.tbz";
      sha256 = "adaca5b25498a607aa68c371eb7ab1105a9360174d8fe8634487bc87ed3173e5";
    };
    opam = "${repo}/packages/albatross/albatross.1.5.0/opam";
    depends = with self; [ asn1-combinators bigstringaf bos checkseum
                           cmdliner cstruct decompress dune dune-configurator
                           duration fmt happy-eyeballs-lwt hex
                           http-lwt-client ipaddr logs lwt metrics
                           metrics-influx metrics-lwt metrics-rusage
                           mirage-crypto mirage-crypto-rng ocaml owee ptime
                           solo5-elftool tls x509 ];
  ...
  metrics-influx = {
    ...
    opam = "${repo}/packages/metrics-influx/metrics-influx.0.4.0/opam";
    depends = with self; [ dune duration fmt lwt metrics ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  metrics-lwt = {
    name = "metrics-lwt";
    ...
    opam = "${repo}/packages/metrics-lwt/metrics-lwt.0.4.0/opam";
    depends = with self; [ dune logs lwt metrics ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  metrics-rusage = {
    name = "metrics-rusage";
    ...
    opam = "${repo}/packages/metrics-rusage/metrics-rusage.0.4.0/opam";
    depends = with self; [ dune fmt logs metrics ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  mirage-clock = {
    name = "mirage-clock";
  ...
  solo5-elftool = {
    ...
    version = "0.3.1";
    src = pkgs.fetchurl {
      url = "https://github.com/roburio/ocaml-solo5-elftool/releases/download/v0.3.1/solo5-elftool-0.3.1.tbz";
      sha256 = "8e8186f006c634fe5a7822f10aead39db7b36d540dbb17a67550252cfcbace8e";
    };
    opam = "${repo}/packages/solo5-elftool/solo5-elftool.0.3.1/opam";
    depends = with self; [ cmdliner cstruct dune fmt ocaml owee ];
    buildDepends = with self; [ dune ocaml ];
  };
  stdlib-shims = {
    name = "stdlib-shims";
