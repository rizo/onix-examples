...
} }:
{
  ...
  ipaddr = {
    ...
    opam = "${repo}/packages/ipaddr/ipaddr.3.1.0/opam";
    depends = with self; [ dune macaddr ocaml sexplib0 ];
    buildDepends = with self; [ dune ocaml ];
  };
  ipv6-multicast = {
    name = "ipv6-multicast";
    version = "0.9";
    src = pkgs.fetchurl {
      url = "https://github.com/vbmithr/ocaml-ipv6-multicast/archive/0.9.tar.gz";
      sha256 = "1n1d2yxsljvgis84m3pfyd8rdljawfcv3k9688b3li22zlb1mi9p";
    };
    opam = "${repo}/packages/ipv6-multicast/ipv6-multicast.0.9/opam";
    depends = with self; [ cstruct ipaddr jbuilder ocaml rresult ];
  ...
  ipv6-multicast-lwt = {
    ...
    opam = "${repo}/packages/ipv6-multicast-lwt/ipv6-multicast-lwt.0.9/opam";
    depends = with self; [ ipv6-multicast jbuilder lwt ocaml ];
    buildDepends = with self; [ ocaml ];
  };
  jbuilder = {
    name = "jbuilder";
  ...
  macaddr = {
    ...
    opam = "${repo}/packages/macaddr/macaddr.3.1.0/opam";
    depends = with self; [ dune ocaml sexplib0 ];
    buildDepends = with self; [ dune ocaml ];
  };
  mmap = {
    name = "mmap";
