...
} }:
{
  ...
  bastet = {
    ...
    version = "2.0.0";
    src = pkgs.fetchurl {
      url = "https://github.com/Risto-Stevcev/bastet/archive/2.0.0.tar.gz";
    ...
    opam = "${repo}/packages/bastet/bastet.2.0.0/opam";
    depends = with self; [ bisect_ppx dune ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  bastet_async = {
    name = "bastet_async";
    version = "0.1.0";
    src = pkgs.fetchurl {
      url = "https://github.com/Risto-Stevcev/bastet-async/archive/0.1.0.tar.gz";
    ...
    opam = "${repo}/packages/bastet_async/bastet_async.0.1.0/opam";
    depends = with self; [ async_kernel bastet dune ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  bin_prot = {
    name = "bin_prot";
  ...
  bisect_ppx = {
    ...
    version = "2.8.1";
    src = pkgs.fetchurl {
      url = "https://github.com/aantron/bisect_ppx/archive/2.8.1.tar.gz";
      sha256 = "14wdqigd98c3fq0hv6sqjwh3hnd9ac0rr2yvpn3i96nlp3d944nm";
    };
    opam = "${repo}/packages/bisect_ppx/bisect_ppx.2.8.1/opam";
    depends = with self; [ base-unix cmdliner dune ocaml ppxlib ];
    buildDepends = with self; [ dune ocaml ];
  };
  cmdliner = {
    name = "cmdliner";
