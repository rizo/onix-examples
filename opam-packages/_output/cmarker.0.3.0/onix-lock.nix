...
} }:
{
  ...
  cmarker = {
    ...
    version = "0.3.0";
    src = pkgs.fetchurl {
      url = "https://github.com/pablo-meier/ocaml-cmark/archive/0.3.0.tar.gz";
    ...
    opam = "${repo}/packages/cmarker/cmarker.0.3.0/opam";
    depends = with self; [ ctypes ctypes-foreign dune ocaml utop ];
    buildDepends = with self; [ dune ocaml ];
    depexts = with pkgs; [ (pkgs.cmark or null) ];
  };
  conf-libffi = {
    name = "conf-libffi";
    version = "2.0.0";
