...
} }:
{
  ...
  current_ocluster = {
    ...
    opam = "${repo}/packages/current_ocluster/current_ocluster.0.1/opam";
    depends = with self; [ capnp-rpc-unix current current_git dune duration
                           fmt logs lwt ocaml ocluster-api ppx_deriving
                           ppx_deriving_yojson prometheus ];
    buildDepends = with self; [ dune ocaml ];
  };
  domain-name = {
    name = "domain-name";
