...
} }:
{
  ...
  conduit-mirage = {
    ...
    opam = "${repo}/packages/conduit-mirage/conduit-mirage.5.1.0/opam";
    depends = with self; [ ca-certs-nss conduit-lwt cstruct dns-client dune
                           fmt ipaddr ipaddr-sexp mirage-clock mirage-flow
                           mirage-flow-combinators mirage-random mirage-time
                           ocaml ppx_sexp_conv sexplib tcpip tls tls-mirage
                           uri vchan xenstore ];
    buildDepends = with self; [ dune ocaml ];
  };
  conf-gmp = {
