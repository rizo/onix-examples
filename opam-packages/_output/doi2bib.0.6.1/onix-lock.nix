...
} }:
{
  ...
  clz = {
    ...
    opam = "${repo}/packages/clz/clz.0.1.0/opam";
    depends = with self; [ bigstringaf cohttp-lwt decompress dune lwt ocaml
                           ptime ];
    buildDepends = with self; [ dune ocaml ];
  };
  cmdliner = {
  ...
  doi2bib = {
    ...
    version = "0.6.1";
    src = pkgs.fetchurl {
      url = "https://github.com/mseri/doi2bib/releases/download/0.6.1/doi2bib-0.6.1.tbz";
      sha256 = "823338a90e259438bb54115ca8ad8265a9db3e88a1fdb5d46dce7ecbe3c8e1e7";
    };
    opam = "${repo}/packages/doi2bib/doi2bib.0.6.1/opam";
    depends = with self; [ astring bigstringaf clz cmdliner cohttp-lwt-unix
                           dune ezxmlm lwt ocaml re tls ];
    buildDepends = with self; [ dune ocaml ];
  };
  domain-name = {
    name = "domain-name";
