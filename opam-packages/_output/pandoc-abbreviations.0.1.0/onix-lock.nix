...
} }:
{
  ...
  pandoc = {
    ...
    version = "0.1.0";
    src = pkgs.fetchurl {
      url = "https://github.com/smimram/ocaml-pandoc/archive/v0.1.0.tar.gz";
    ...
    opam = "${repo}/packages/pandoc/pandoc.0.1.0/opam";
    depends = with self; [ dune yojson ];
  ...
  pandoc-abbreviations = {
    ...
    opam = "${repo}/packages/pandoc-abbreviations/pandoc-abbreviations.0.1.0/opam";
    depends = with self; [ dune ocaml pandoc ];
    buildDepends = with self; [ dune ocaml ];
  };
  yojson = {
    name = "yojson";
