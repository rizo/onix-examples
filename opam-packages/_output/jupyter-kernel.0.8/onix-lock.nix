...
} }:
{
  ...
  jupyter-kernel = {
    ...
    version = "0.8";
    src = pkgs.fetchurl {
      url = "https://github.com/ocaml-jupyter/jupyter-kernel/archive/v0.8.tar.gz";
    ...
    opam = "${repo}/packages/jupyter-kernel/jupyter-kernel.0.8/opam";
    depends = with self; [ atdgen base-bytes base-unix digestif dune logs lwt
                           ocaml ptime result uuidm uutf yojson zmq zmq-lwt
                           (self.tyxml or null) ];
    buildDepends = with self; [ dune ocaml ];
  };
  logs = {
