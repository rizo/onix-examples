...
} }:
{
  ...
  conf-mariadb = {
    name = "conf-mariadb";
    version = "2";
    opam = "${repo}/packages/conf-mariadb/conf-mariadb.2/opam";
    buildDepends = with self; [ conf-pkg-config ];
    depexts = with pkgs; [ mariadb mariadb-connector-c ];
  };
  conf-mysql = {
    name = "conf-mysql";
    version = "1";
  ...
  links = {
    ...
    version = "0.9.6";
    src = pkgs.fetchurl {
      url = "https://github.com/links-lang/links/releases/download/0.9.6/links-0.9.6.tbz";
      sha256 = "1d573072729709287b40068971d9a0a1ed0490d24af8d8c2667f5dd174dd19c1";
    };
    opam = "${repo}/packages/links/links.0.9.6/opam";
    depends = with self; [ ANSITerminal base64 calendar cohttp cohttp-lwt
                           cohttp-lwt-unix conduit-lwt-unix dune linenoise
                           lwt menhir ocaml ocamlfind ppx_deriving
                           ppx_deriving_yojson ppx_sexp_conv result safepass
                           tls uri websocket websocket-lwt-unix ];
  ...
  links-mysql = {
    ...
    opam = "${repo}/packages/links-mysql/links-mysql.0.9.6/opam";
    depends = with self; [ conf-mysql dune links mysql ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  logs = {
    name = "logs";
  ...
  mysql = {
    ...
    version = "1.2.4";
    src = pkgs.fetchurl {
      url = "https://ygrek.org/p/release/ocaml-mysql/ocaml-mysql-1.2.4.tar.gz";
      sha256 = "8535a88f74a34dfdb51c4539e4a965b84204273bd04e539a5d729f2536da6c9f";
    };
    opam = "${repo}/packages/mysql/mysql.1.2.4/opam";
    depends = with self; [ ocaml (self.conf-mariadb or null)
                           (self.conf-mysql or null) ];
    buildDepends = with self; [ ocaml ocamlfind ];
  };
  num = {
    name = "num";
