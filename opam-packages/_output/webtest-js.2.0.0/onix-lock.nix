...
} }:
{
  ...
  webtest = {
    ...
    version = "2.0.0";
    src = pkgs.fetchurl {
      url = "https://github.com/johnelse/ocaml-webtest/archive/webtest.2.0.0.tar.gz";
      sha256 = "1b2ar0p3sbha4clf7n34xghq14f8xwqwfhf8xlmvlhl615290s41";
    };
    opam = "${repo}/packages/webtest/webtest.2.0.0/opam";
    depends = with self; [ dune ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  webtest-js = {
    ...
    opam = "${repo}/packages/webtest-js/webtest-js.2.0.0/opam";
    depends = with self; [ dune js_of_ocaml js_of_ocaml-ppx ocaml webtest ];
    buildDepends = with self; [ dune ocaml ];
  };
  yojson = {
    name = "yojson";
