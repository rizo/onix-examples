...
} }:
{
  ...
  canary = {
    ...
    version = "0.0.1";
    src = pkgs.fetchurl {
      url = "https://github.com/chrismamo1/canary/archive/0.0.1.tar.gz";
    ...
    opam = "${repo}/packages/canary/canary.0.0.1/opam";
    depends = with self; [ cohttp cohttp-lwt cohttp-lwt-unix dune lwt lwt_ppx
                           ocaml ppx_deriving_yojson ptime uri yojson ];
    buildDepends = with self; [ dune ocaml ];
  };
  cmdliner = {
    name = "cmdliner";
