...
} }:
{
  ...
  hvsock = {
    ...
    version = "3.0.0";
    src = pkgs.fetchurl {
      url = "https://github.com/mirage/ocaml-hvsock/releases/download/3.0.0/hvsock-3.0.0.tbz";
      sha256 = "46d179976f153a7723890ba78fbcfee60e70b3cc9cc3574eb4bb8a682525e4e2";
    };
    opam = "${repo}/packages/hvsock/hvsock.3.0.0/opam";
    depends = with self; [ base-bytes base-threads base-unix base64 cmdliner
                           cstruct dune duration fmt logs lwt mirage-flow
                           mirage-flow-combinators mirage-time ocaml sha uri
                           uuidm uutf ];
    buildDepends = with self; [ dune ocaml ];
