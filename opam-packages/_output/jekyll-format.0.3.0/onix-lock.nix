...
} }:
{
  ...
  jekyll-format = {
    ...
    version = "0.3.0";
    src = pkgs.fetchurl {
      url = "https://github.com/avsm/jekyll-format/releases/download/v0.3.0/jekyll-format-v0.3.0.tbz";
      sha256 = "129fb63081b8dee1f4d44f5e8ef28d9e4fe8d7661f6371d35d1ee8038e1c792a";
    };
    opam = "${repo}/packages/jekyll-format/jekyll-format.0.3.0/opam";
    depends = with self; [ astring dune ezjsonm fmt fpath ocaml omd ptime
                           result rresult sexplib yaml yaml-sexp ];
    buildDepends = with self; [ dune ocaml ];
  };
  jsonm = {
    name = "jsonm";
  ...
  yaml-sexp = {
    ...
    opam = "${repo}/packages/yaml-sexp/yaml-sexp.3.1.0/opam";
    depends = with self; [ dune ppx_sexp_conv sexplib yaml ];
