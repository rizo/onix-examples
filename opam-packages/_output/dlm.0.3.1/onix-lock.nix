...
} }:
{
  ...
  dlm = {
    ...
    version = "0.3.1";
    src = pkgs.fetchurl {
      url = "https://github.com/xapi-project/ocaml-dlm/archive/v0.3.1.tar.gz";
      sha256 = "1igd0gzwb3y3bbc0dzmvdlx451ddd1pllgahh3s4gfz0nlbdbnrm";
    };
    opam = "${repo}/packages/dlm/dlm.0.3.1/opam";
    depends = with self; [ conf-pkg-config ctypes dune lwt ocaml ounit
                           unix-errno uuidm ];
    buildDepends = with self; [ dune ocaml ];
    depexts = with pkgs; [ (pkgs.dlm-devel or null) (pkgs.libdlm-dev or null)
                           (pkgs.libdlm-devel or null) ];
  };
  dune = {
    name = "dune";
    version = "3.2.0";
