...
} }:
{
  ...
  bdd = {
    ...
    opam = "${repo}/packages/bdd/bdd.0.3/opam";
    depends = with self; [ obuild ocaml ocamlfind ];
    buildDepends = with self; [ ocaml ocamlfind ];
  };
  camlidl = {
    name = "camlidl";
  ...
  clangml = {
    ...
    version = "4.5.0";
    src = pkgs.fetchurl {
      url = "https://gitlab.inria.fr/memcad/clangml/uploads/888b796e6a42405e831315d967bf094d/clangml.4.5.0.tar.gz";
    ...
    opam = "${repo}/packages/clangml/clangml.4.5.0/opam";
  ...
  clangml-transforms = {
    ...
    version = "0.26";
    src = pkgs.fetchurl {
      url = "https://gitlab.inria.fr/memcad/clangml-transforms/-/archive/v0.26/clangml-transforms-v0.26.tar.gz";
    ...
    opam = "${repo}/packages/clangml-transforms/clangml-transforms.0.26/opam";
    depends = with self; [ clangml dolog dune ocaml refl traverse ];
  ...
  conf-libclang = {
    name = "conf-libclang";
    version = "13";
    opam = "${repo}/packages/conf-libclang/conf-libclang.13/opam";
  ...
  memcad = {
    ...
    version = "1.1.0";
    src = pkgs.fetchurl {
      url = "https://gitlab.inria.fr/memcad/memcad/-/archive/v1.1.0/memcad-v1.1.0.tar.gz";
    ...
    opam = "${repo}/packages/memcad/memcad.1.1.0/opam";
    depends = with self; [ apron base-unix bdd clangml clangml-transforms
                           conf-graphviz dune mlbdd ocaml ounit parmap qtest
                           setr ];
    buildDepends = with self; [ dune ocaml ];
  };
  metapp = {
  ...
  mlbdd = {
    ...
    version = "0.7.3";
    src = pkgs.fetchurl {
      url = "https://github.com/arlencox/mlbdd/archive/v0.7.3.tar.gz";
    ...
    opam = "${repo}/packages/mlbdd/mlbdd.0.7.3/opam";
    depends = with self; [ dune ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  mlgmpidl = {
  ...
  qtest = {
    ...
    version = "2.11.2";
    src = pkgs.fetchurl {
      url = "https://github.com/vincent-hugot/qtest/archive/v2.11.2.tar.gz";
    ...
    opam = "${repo}/packages/qtest/qtest.2.11.2/opam";
    depends = with self; [ base-bytes dune ocaml ounit2 qcheck ];
    buildDepends = with self; [ dune ocaml ];
  };
  refl = {
    name = "refl";
  ...
  setr = {
    ...
    opam = "${repo}/packages/setr/setr.0.1.1/opam";
    depends = with self; [ mlbdd ocaml ];
