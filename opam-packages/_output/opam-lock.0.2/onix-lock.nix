...
} }:
{
  ...
  opam-lock = {
    ...
    version = "0.2";
    src = pkgs.fetchurl {
      url = "https://github.com/AltGr/opam-lock/archive/0.2.tar.gz";
    ...
    opam = "${repo}/packages/opam-lock/opam-lock.0.2/opam";
    depends = with self; [ cmdliner jbuilder ocaml opam-solver opam-state ];
    buildDepends = with self; [ ocaml ];
  };
  opam-repository = {
