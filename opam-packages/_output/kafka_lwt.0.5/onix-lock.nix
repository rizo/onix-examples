...
} }:
{
  ...
  kafka = {
    ...
    opam = "${repo}/packages/kafka/kafka.0.5/opam";
    depends = with self; [ conf-zlib dune ocaml ];
    buildDepends = with self; [ dune ocaml ];
    depexts = with pkgs; [ (pkgs.devel/librdkafka or null)
                           (pkgs.librdkafka or null)
                           (pkgs.librdkafka-dev or null)
                           (pkgs.librdkafka-devel or null)
                           (pkgs.net/librdkafka or null) ];
  };
  kafka_lwt = {
    name = "kafka_lwt";
    version = "0.5";
    ...
    opam = "${repo}/packages/kafka_lwt/kafka_lwt.0.5/opam";
    depends = with self; [ cmdliner conf-zlib dune kafka lwt ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  lwt = {
    name = "lwt";
