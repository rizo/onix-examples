...
} }:
{
  ...
  opam-grep = {
    ...
    version = "0.3.0";
    src = pkgs.fetchurl {
      url = "https://github.com/kit-ty-kate/opam-grep/releases/download/v0.3.0/opam-grep-0.3.0.tar.gz";
    ...
    opam = "${repo}/packages/opam-grep/opam-grep.0.3.0/opam";
    depends = with self; [ bos cmdliner dune fpath ocaml progress ];
