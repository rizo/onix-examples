...
} }:
{
  ...
  proc-smaps = {
    ...
    version = "0.1.0";
    src = pkgs.fetchurl {
      url = "https://gitlab.com/dailambda/proc-smaps/-/archive/0.1.0/proc-smaps-0.1.0.tar.bz2";
    ...
    opam = "${repo}/packages/proc-smaps/proc-smaps.0.1.0/opam";
    depends = with self; [ dune lwt ocaml stdint ];
    buildDepends = with self; [ dune ocaml ];
  };
  result = {
    name = "result";
