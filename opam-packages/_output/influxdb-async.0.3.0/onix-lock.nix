...
} }:
{
  ...
  influxdb = {
    ...
    version = "0.3.0";
    src = pkgs.fetchurl {
      url = "https://github.com/ryanslade/influxdb-ocaml/archive/0.3.0.tar.gz";
    ...
    opam = "${repo}/packages/influxdb/influxdb.0.3.0/opam";
    depends = with self; [ base dune ocaml ppx_expect stdio ];
    buildDepends = with self; [ dune ocaml ];
  };
  influxdb-async = {
    name = "influxdb-async";
    ...
    opam = "${repo}/packages/influxdb-async/influxdb-async.0.3.0/opam";
    depends = with self; [ async base cohttp cohttp-async dune influxdb ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  int_repr = {
    name = "int_repr";
