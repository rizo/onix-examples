...
} }:
{
  ...
  conf-opencc0 = {
    name = "conf-opencc0";
    version = "1";
    opam = "${repo}/packages/conf-opencc0/conf-opencc0.1/opam";
    depexts = with pkgs; [ (pkgs.libopencc1 or null) ];
  };
  conf-pkg-config = {
  ...
  opencc0 = {
    ...
    version = "1.0.2";
    src = pkgs.fetchurl {
      url = "https://github.com/kandu/ocaml-opencc0/archive/refs/tags/1.0.2.tar.gz";
      sha256 = "1nfbdwkwpl1ssh6nijjkjxvsyvisr9h01np3bam0s9amprwj4y1h";
    };
    opam = "${repo}/packages/opencc0/opencc0.1.0.2/opam";
    depends = with self; [ conf-opencc0 ctypes ctypes-foreign dune ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  stdlib-shims = {
    name = "stdlib-shims";
