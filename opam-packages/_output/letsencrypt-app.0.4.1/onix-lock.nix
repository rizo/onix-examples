...
} }:
{
  ...
  letsencrypt-app = {
    ...
    opam = "${repo}/packages/letsencrypt-app/letsencrypt-app.0.4.1/opam";
    depends = with self; [ bos cmdliner cohttp-lwt-unix cstruct dune fmt
                           fpath letsencrypt letsencrypt-dns logs lwt
                           mirage-crypto-rng ocaml ptime randomconv ];
    buildDepends = with self; [ dune ocaml ];
  };
  letsencrypt-dns = {
    ...
    opam = "${repo}/packages/letsencrypt-dns/letsencrypt-dns.0.4.1/opam";
    depends = with self; [ dns dns-tsig domain-name dune fmt letsencrypt logs
                           lwt ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  logs = {
