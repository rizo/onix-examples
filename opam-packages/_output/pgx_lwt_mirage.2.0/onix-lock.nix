...
} }:
{
  ...
  pgx = {
    ...
    version = "2.0";
    src = pkgs.fetchurl {
      url = "https://github.com/arenadotio/pgx/releases/download/2.0/pgx_lwt_mirage-2.0.tbz";
      sha256 = "be6bac83e4030b6225f3966fd482b242818ff1147efc273163d4a9cd749b62bd";
    };
    opam = "${repo}/packages/pgx/pgx.2.0/opam";
    depends = with self; [ dune hex ipaddr ocaml ppx_compare
                           ppx_custom_printf ppx_sexp_conv re sexplib0 uuidm ];
    buildDepends = with self; [ dune ocaml ];
  };
  pgx_lwt = {
    name = "pgx_lwt";
    ...
    opam = "${repo}/packages/pgx_lwt/pgx_lwt.2.0/opam";
    depends = with self; [ dune logs lwt ocaml pgx ];
    buildDepends = with self; [ dune ocaml ];
  };
  pgx_lwt_mirage = {
    name = "pgx_lwt_mirage";
    ...
    opam = "${repo}/packages/pgx_lwt_mirage/pgx_lwt_mirage.2.0/opam";
    depends = with self; [ conduit-mirage dns-client dune logs lwt
                           mirage-channel mirage-clock mirage-random
                           mirage-stack mirage-time ocaml pgx pgx_lwt ];
    buildDepends = with self; [ dune ocaml ];
  };
  ppx_compare = {
    name = "ppx_compare";
