...
} }:
{
  ...
  dns-certify = {
    ...
    opam = "${repo}/packages/dns-certify/dns-certify.6.2.2/opam";
    depends = with self; [ dns dns-mirage dns-tsig dune duration logs lwt
                           mirage-clock mirage-crypto-ec mirage-crypto-pk
                           mirage-crypto-rng mirage-random mirage-time ocaml
                           randomconv tcpip x509 ];
    buildDepends = with self; [ dune ocaml ];
  };
  dns-cli = {
    name = "dns-cli";
    ...
    opam = "${repo}/packages/dns-cli/dns-cli.6.2.2/opam";
    depends = with self; [ bos cmdliner cstruct dns dns-certify dns-client
                           dns-server dns-tsig dnssec dune fmt fpath hex
                           ipaddr logs lwt mirage-crypto mirage-crypto-pk
                           mirage-crypto-rng mtime ocaml ptime randomconv
                           x509 ];
    buildDepends = with self; [ dune ocaml ];
  };
  dns-client = {
  ...
  dns-mirage = {
    ...
    opam = "${repo}/packages/dns-mirage/dns-mirage.6.2.2/opam";
    depends = with self; [ cstruct dns dune ipaddr lwt ocaml tcpip ];
    buildDepends = with self; [ dune ocaml ];
  };
  dns-server = {
    name = "dns-server";
    ...
    opam = "${repo}/packages/dns-server/dns-server.6.2.2/opam";
    depends = with self; [ cstruct dns dns-mirage dune duration lwt metrics
                           mirage-clock mirage-time ocaml randomconv tcpip ];
    buildDepends = with self; [ dune ocaml ];
  };
  dns-tsig = {
    name = "dns-tsig";
  ...
  dnssec = {
    ...
    opam = "${repo}/packages/dnssec/dnssec.6.2.2/opam";
    depends = with self; [ cstruct dns domain-name dune logs mirage-crypto
                           mirage-crypto-ec mirage-crypto-pk ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  domain-name = {
    name = "domain-name";
