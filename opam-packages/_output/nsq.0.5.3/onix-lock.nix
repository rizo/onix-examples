...
} }:
{
  ...
  nsq = {
    ...
    version = "0.5.3";
    src = pkgs.fetchurl {
      url = "https://github.com/ryanslade/nsq-ocaml/archive/0.5.3.tar.gz";
    ...
    opam = "${repo}/packages/nsq/nsq.0.5.3/opam";
    depends = with self; [ base cohttp cohttp-lwt-unix dune hex integers logs
                           lwt ocaml ocaml-migrate-parsetree ocplib-endian
                           ppx_deriving_yojson ppx_jane stdio yojson ];
    buildDepends = with self; [ dune ocaml ];
  };
  num = {
    name = "num";
