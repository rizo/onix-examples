...
} }:
{
  ...
  cstruct-async = {
    ...
    opam = "${repo}/packages/cstruct-async/cstruct-async.6.1.0/opam";
    depends = with self; [ async async_unix core cstruct dune ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  cstruct-sexp = {
    name = "cstruct-sexp";
  ...
  mirage-crypto-rng-async = {
    name = "mirage-crypto-rng-async";
    version = "0.10.6";
    src = pkgs.fetchurl {
      url = "https://github.com/mirage/mirage-crypto/releases/download/v0.10.6/mirage-crypto-0.10.6.tbz";
      sha256 = "01d6477a4edcad007b56983955d327f0e61c3f36494822f3755017d26e8f9410";
    };
    opam = "${repo}/packages/mirage-crypto-rng-async/mirage-crypto-rng-async.0.10.6/opam";
    depends = with self; [ async dune dune-configurator logs mirage-crypto
                           mirage-crypto-rng ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  mirage-no-solo5 = {
  ...
  tls-async = {
    ...
    opam = "${repo}/packages/tls-async/tls-async.0.15.3/opam";
    depends = with self; [ async async_unix core cstruct-async dune
                           mirage-crypto-rng-async ocaml ppx_jane ptime tls
                           x509 ];
    buildDepends = with self; [ dune ocaml ];
  };
  topkg = {
