...
} }:
{
  ...
  conf-srt = {
    name = "conf-srt";
    version = "1";
    opam = "${repo}/packages/conf-srt/conf-srt.1/opam";
    buildDepends = with self; [ conf-pkg-config ];
    depexts = with pkgs; [ srt ];
  };
  csexp = {
    name = "csexp";
    version = "1.5.1";
  ...
  srt = {
    ...
    version = "0.2.1";
    src = pkgs.fetchurl {
      url = "https://github.com/savonet/ocaml-srt/archive/v0.2.1.tar.gz";
    ...
    opam = "${repo}/packages/srt/srt.0.2.1/opam";
    depends = with self; [ ctypes dune integers posix-socket ];
    buildDepends = with self; [ conf-pkg-config conf-srt dune
                                dune-configurator ];
  };
  stdlib-shims = {
    name = "stdlib-shims";
