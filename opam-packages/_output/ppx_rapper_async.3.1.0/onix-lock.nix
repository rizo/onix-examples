...
} }:
{
  ...
  caqti-async = {
    ...
    opam = "${repo}/packages/caqti-async/caqti-async.1.8.0/opam";
    depends = with self; [ async_kernel async_unix caqti core_kernel dune
                           ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  cmdliner = {
  ...
  pg_query = {
    ...
    opam = "${repo}/packages/pg_query/pg_query.0.9.7/opam";
    depends = with self; [ cmdliner ctypes ctypes-foreign dune ocaml
                           ppx_deriving ];
    buildDepends = with self; [ dune ocaml ];
  };
  ppx_assert = {
  ...
  ppx_rapper = {
    ...
    version = "3.1.0";
    src = pkgs.fetchurl {
      url = "https://github.com/roddyyaga/ppx_rapper/archive/3.1.0.tar.gz";
    ...
    opam = "${repo}/packages/ppx_rapper/ppx_rapper.3.1.0/opam";
    depends = with self; [ base caqti dune ocaml pg_query ppxlib ];
    buildDepends = with self; [ dune ocaml ];
  };
  ppx_rapper_async = {
    ...
    opam = "${repo}/packages/ppx_rapper_async/ppx_rapper_async.3.1.0/opam";
    depends = with self; [ async caqti-async dune ocaml ppx_rapper ];
    buildDepends = with self; [ dune ocaml ];
  };
  ppx_sexp_conv = {
    name = "ppx_sexp_conv";
