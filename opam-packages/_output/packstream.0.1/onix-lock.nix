...
} }:
{
  ...
  packstream = {
    ...
    version = "0.1";
    src = pkgs.fetchurl {
      url = "https://github.com/tomob/packstream/archive/0.1.tar.gz";
    ...
    opam = "${repo}/packages/packstream/packstream.0.1/opam";
    depends = with self; [ bitstring core dune ocaml ppx_bitstring
                           ppx_deriving ];
    buildDepends = with self; [ dune ocaml ];
  };
  parsexp = {
  ...
  ppx_bitstring = {
    ...
    opam = "${repo}/packages/ppx_bitstring/ppx_bitstring.4.1.0/opam";
    depends = with self; [ bitstring dune ocaml ppxlib ];
    buildDepends = with self; [ dune ocaml ];
  };
  ppx_cold = {
    name = "ppx_cold";
