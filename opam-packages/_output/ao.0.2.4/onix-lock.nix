...
} }:
{
  ao = {
    ...
    opam = "${repo}/packages/ao/ao.0.2.4/opam";
    depends = with self; [ conf-ao conf-pkg-config dune dune-configurator ];
  ...
  conf-ao = {
    name = "conf-ao";
    version = "1";
    opam = "${repo}/packages/conf-ao/conf-ao.1/opam";
    buildDepends = with self; [ conf-pkg-config ];
    depexts = with pkgs; [ libao ];
  };
  conf-pkg-config = {
