...
} }:
{
  ...
  kubecaml = {
    ...
    opam = "${repo}/packages/kubecaml/kubecaml.0.1.0/opam";
    depends = with self; [ cohttp-lwt-unix jbuilder lwt ocaml
                           ocaml-migrate-parsetree ppx_deriving_yojson re
                           swagger uri yojson ];
    buildDepends = with self; [ ocaml ];
  };
  logs = {
  ...
  ppx_deriving_yojson = {
    ...
    opam = "${repo}/packages/ppx_deriving_yojson/ppx_deriving_yojson.3.5.3/opam";
    depends = with self; [ dune ocaml ppx_deriving ppx_tools result yojson ];
    buildDepends = with self; [ cppo dune ocaml ppxfind ];
  };
  ppx_sexp_conv = {
    name = "ppx_sexp_conv";
