...
} }:
{
  ...
  aws = {
    ...
    version = "1.2";
    src = pkgs.fetchurl {
      url = "https://github.com/inhabitedtype/ocaml-aws/archive/v1.2.tar.gz";
    ...
    opam = "${repo}/packages/aws/aws.1.2/opam";
    depends = with self; [ base64 calendar digestif dune ezxmlm ocaml uri ];
  ...
  aws-elasticloadbalancing = {
    ...
    opam = "${repo}/packages/aws-elasticloadbalancing/aws-elasticloadbalancing.1.2/opam";
    depends = with self; [ aws dune ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  base-bytes = {
    name = "base-bytes";
