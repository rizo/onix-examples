...
} }:
{
  ...
  tls = {
    ...
    opam = "${repo}/packages/tls/tls.0.9.3/opam";
    depends = with self; [ cstruct nocrypto ocaml ppx_cstruct ppx_deriving
                           ppx_sexp_conv result sexplib x509
  ...
  x509 = {
    ...
    opam = "${repo}/packages/x509/x509.0.6.2/opam";
    depends = with self; [ asn1-combinators astring cstruct nocrypto ocaml
                           ppx_sexp_conv ptime result sexplib ];
    buildDepends = with self; [ ocaml ocamlbuild ocamlfind topkg ];
  };
  zarith = {
    name = "zarith";
