...
} }:
{
  ...
  index-bench = {
    ...
    opam = "${repo}/packages/index-bench/index-bench.1.6.0/opam";
    depends = with self; [ cmdliner dune fmt index logs metrics metrics-unix
                           mtime ocaml ppx_deriving_yojson ppx_repr progress
                           re stdlib-shims yojson ];
    buildDepends = with self; [ dune ocaml ];
  };
  jsonm = {
    name = "jsonm";
