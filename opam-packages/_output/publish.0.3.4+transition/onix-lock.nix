...
} }:
{
  ...
  opam-publish = {
    ...
    version = "2.1.0";
    src = pkgs.fetchurl {
      url = "https://github.com/ocaml/opam-publish/archive/2.1.0.tar.gz";
    ...
    opam = "${repo}/packages/opam-publish/opam-publish.2.1.0/opam";
    depends = with self; [ cmdliner dune github github-unix lwt_ssl ocaml
                           opam-core opam-format opam-state ];
    buildDepends = with self; [ dune ocaml ];
  };
  opam-repository = {
  ...
  publish = {
    name = "publish";
    version = "0.3.4+transition";
    opam = "${repo}/packages/publish/publish.0.3.4+transition/opam";
    depends = with self; [ ocaml opam-publish ];
    buildDepends = with self; [ ocaml ];
  };
  re = {
    name = "re";
