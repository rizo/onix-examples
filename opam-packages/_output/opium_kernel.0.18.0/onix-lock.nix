...
} }:
{
  ...
  opium_kernel = {
    ...
    opam = "${repo}/packages/opium_kernel/opium_kernel.0.18.0/opam";
    depends = with self; [ cohttp cohttp-lwt dune fieldslib hmap lwt ocaml
                           ppx_fields_conv ppx_sexp_conv re sexplib ];
    buildDepends = with self; [ dune ocaml ];
  };
  parsexp = {
    name = "parsexp";
