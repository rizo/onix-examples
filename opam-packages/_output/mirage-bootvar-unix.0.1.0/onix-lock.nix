...
} }:
{
  ...
  mirage-bootvar-unix = {
    name = "mirage-bootvar-unix";
    version = "0.1.0";
    src = pkgs.fetchurl {
      url = "https://github.com/mirage/mirage-bootvar-unix/releases/download/0.1.0/mirage-bootvar-unix-0.1.0.tbz";
      sha256 = "0r92s6y7nxg0ci330a7p0hii4if51iq0sixn20cnm5j4a2clprbf";
    };
    opam = "${repo}/packages/mirage-bootvar-unix/mirage-bootvar-unix.0.1.0/opam";
    depends = with self; [ dune lwt ocaml parse-argv ];
    buildDepends = with self; [ dune ocaml ];
  };
  mmap = {
    name = "mmap";
