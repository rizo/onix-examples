...
} }:
{
  ...
  conf-llvm = {
    name = "conf-llvm";
    version = "13.0.0";
    opam = "${repo}/packages/conf-llvm/conf-llvm.13.0.0/opam";
    depexts = with pkgs; [ (pkgs.devel/llvm13 or null)
                           (pkgs.epel-release or null) (pkgs.llvm-13 or null)
                           (pkgs.llvm-13-dev or null)
                           (pkgs.llvm13-dev or null)
                           (pkgs.llvm13-devel or null) (pkgs.llvm@13 or null) ];
  };
  conf-python-3 = {
    name = "conf-python-3";
    version = "1.0.0";
  ...
  llopt = {
    ...
    version = "1.0.0";
    src = pkgs.fetchurl {
      url = "https://github.com/kit-ty-kate/llopt/archive/1.0.0.tar.gz";
      sha256 = "0m82awl6lccsw3g0q50v9b1bscgknhdr1hnsfbzpqw4yzx8znzs5";
    };
    opam = "${repo}/packages/llopt/llopt.1.0.0/opam";
    depends = with self; [ cmdliner jbuilder llvm ocaml ];
    buildDepends = with self; [ ocaml ];
  };
  llvm = {
    name = "llvm";
    ...
    opam = "${repo}/packages/llvm/llvm.13.0.0/opam";
    depends = with self; [ ctypes ocaml ];
    buildDepends = with self; [ conf-cmake conf-llvm conf-python-3 ocaml
                                ocamlfind ];
  };
  ocaml = {
    name = "ocaml";
