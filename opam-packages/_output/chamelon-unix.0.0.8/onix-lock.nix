...
} }:
{
  ...
  chamelon-unix = {
    ...
    opam = "${repo}/packages/chamelon-unix/chamelon-unix.0.0.8/opam";
    depends = with self; [ bos chamelon cmdliner dune fmt logs lwt
                           mirage-block mirage-block-unix mirage-clock
                           mirage-clock-unix mirage-kv mirage-logs ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  checkseum = {
    name = "checkseum";
