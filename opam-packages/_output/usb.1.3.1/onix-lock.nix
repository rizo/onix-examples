...
} }:
{
  ...
  usb = {
    ...
    version = "1.3.1";
    src = pkgs.fetchurl {
      url = "https://github.com/letoh/ocaml-usb/archive/v1.3.1.tar.gz";
      sha256 = "1wnzih7f9gxxdwp6sjdywpcdan28j4z1dmbh4djzlznc6f7f2hqm";
    };
    opam = "${repo}/packages/usb/usb.1.3.1/opam";
    depends = with self; [ ocaml (self.lwt or null) (self.lwt_ppx or null) ];
    buildDepends = with self; [ conf-pkg-config oasis ocaml ocamlbuild
                                ocamlfind ];
    depexts = with pkgs; [ (pkgs.libusb or null)
                           (pkgs.libusb-1.0-0-dev or null)
                           (pkgs.libusb-1_0-devel or null)
                           (pkgs.libusb-dev or null)
                           (pkgs.libusb1-devel or null)
                           (pkgs.libusb1.0-devel or null) ];
  };
}
