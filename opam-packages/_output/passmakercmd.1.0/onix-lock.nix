...
} }:
{
  ...
  passmaker = {
    ...
    version = "1.0";
    src = pkgs.fetchurl {
      url = "https://github.com/darioteixeira/passmaker/archive/v1.0.tar.gz";
      sha256 = "1z0zhynmrzkjzs1ljqh815s8qpl5lrj9sadv273fbkarf8fj4hi6";
    };
    opam = "${repo}/packages/passmaker/passmaker.1.0/opam";
    depends = with self; [ jbuilder ocaml ocaml-migrate-parsetree ppx_blob ];
    buildDepends = with self; [ ocaml ];
  };
  passmakercmd = {
    name = "passmakercmd";
    ...
    opam = "${repo}/packages/passmakercmd/passmakercmd.1.0/opam";
    depends = with self; [ cmdliner jbuilder ocaml passmaker rresult ];
    buildDepends = with self; [ ocaml ];
  };
  ppx_blob = {
    name = "ppx_blob";
