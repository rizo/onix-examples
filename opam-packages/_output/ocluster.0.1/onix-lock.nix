...
} }:
{
  ...
  obuilder = {
    ...
    version = "0.3";
    src = pkgs.fetchurl {
      url = "https://github.com/ocurrent/obuilder/releases/download/v0.3/obuilder-spec-v0.3.tbz";
      sha256 = "43473944fefea40e80cfa18461e23e69b5c3b81add828a70250d085bc794d62d";
    };
    opam = "${repo}/packages/obuilder/obuilder.0.3/opam";
    depends = with self; [ astring cmdliner dune fmt logs lwt obuilder-spec
                           ocaml ppx_deriving ppx_sexp_conv sexplib sha
                           sqlite3 tar-unix yojson ];
    buildDepends = with self; [ dune ocaml ];
  };
  obuilder-spec = {
    name = "obuilder-spec";
    ...
    opam = "${repo}/packages/obuilder-spec/obuilder-spec.0.3/opam";
    depends = with self; [ astring dune fmt ocaml ppx_deriving ppx_sexp_conv
                           sexplib ];
    buildDepends = with self; [ dune ocaml ];
  };
  ocaml = {
  ...
  ocluster = {
    ...
    version = "0.1";
    src = pkgs.fetchurl {
      url = "https://github.com/ocurrent/ocluster/releases/download/v0.1/current_ocluster-v0.1.tbz";
      sha256 = "bb303471ffc41350e2a3ce157c1beeffb5aec9799b63118be7c4b8d8c7258a1d";
    };
    opam = "${repo}/packages/ocluster/ocluster.0.1/opam";
    depends = with self; [ capnp-rpc-lwt capnp-rpc-net capnp-rpc-unix
                           cohttp-lwt-unix conf-libev digestif dune
                           dune-build-info fmt fpath logs lwt lwt-dllist
                           mirage-crypto obuilder ocaml ocluster-api
                           ppx_sexp_conv prometheus prometheus-app psq
                           sqlite3 ];
    buildDepends = with self; [ dune ocaml ];
  };
  ocluster-api = {
    ...
    opam = "${repo}/packages/ocluster-api/ocluster-api.0.1/opam";
    depends = with self; [ capnp-rpc-lwt dune fmt lwt ocaml ppx_deriving
                           ppx_deriving_yojson ];
    buildDepends = with self; [ dune ocaml ];
  };
  ocplib-endian = {
  ...
  tar = {
    ...
    version = "2.0.1";
    src = pkgs.fetchurl {
      url = "https://github.com/mirage/ocaml-tar/releases/download/v2.0.1/tar-mirage-2.0.1.tbz";
      sha256 = "e9565fb43dab9e944d1860f9a3b5c681df72211b859c66ee1d7085cafc13ee9b";
    };
    opam = "${repo}/packages/tar/tar.2.0.1/opam";
    depends = with self; [ camlp-streams cstruct dune ocaml re ];
  ...
  tar-unix = {
    ...
    opam = "${repo}/packages/tar-unix/tar-unix.2.0.1/opam";
