...
} }:
{
  ...
  charrua-server = {
    ...
    opam = "${repo}/packages/charrua-server/charrua-server.1.5.0/opam";
    depends = with self; [ charrua cstruct dune ipaddr ipaddr-sexp macaddr
                           macaddr-sexp ocaml ppx_sexp_conv sexplib ];
  ...
  charrua-unix = {
    ...
    opam = "${repo}/packages/charrua-unix/charrua-unix.1.5.0/opam";
    depends = with self; [ charrua charrua-server cmdliner cstruct-lwt
                           cstruct-unix dune ipaddr lwt lwt_log mtime ocaml
                           rawlink tcpip tuntap ];
    buildDepends = with self; [ dune ocaml ];
  };
  cmdliner = {
    name = "cmdliner";
  ...
  rawlink = {
    ...
    version = "1.2";
    src = pkgs.fetchurl {
      url = "https://github.com/haesbaert/rawlink/releases/download/v1.2/rawlink-1.2.tbz";
      sha256 = "a712be95f636466536327fd92ce476fb72a4e06e1b82a85d5b471db8289a91e4";
    };
    opam = "${repo}/packages/rawlink/rawlink.1.2/opam";
  ...
  tuntap = {
    ...
    version = "2.0.0";
    src = pkgs.fetchurl {
      url = "https://github.com/mirage/ocaml-tuntap/releases/download/v2.0.0/tuntap-v2.0.0.tbz";
      sha256 = "4abba9727156222a261e0daecfa54d4c337f5c690870e7f98c5f0e8884a6958b";
    };
    opam = "${repo}/packages/tuntap/tuntap.2.0.0/opam";
    depends = with self; [ cmdliner dune ipaddr macaddr ocaml ];
    buildDepends = with self; [ dune ocaml ];
    depexts = with pkgs; [ (pkgs.linux-headers or null) ];
  };
}
