...
} }:
{
  ...
  angstrom-lwt-unix = {
    ...
    opam = "${repo}/packages/angstrom-lwt-unix/angstrom-lwt-unix.0.15.0/opam";
    depends = with self; [ angstrom base-unix dune lwt ocaml ];
  ...
  dap = {
    ...
    opam = "${repo}/packages/dap/dap.1.0.6/opam";
    depends = with self; [ angstrom angstrom-lwt-unix dune logs lwt lwt_ppx
                           lwt_react ocaml ppx_deriving ppx_deriving_yojson
                           ppx_expect ppx_here react yojson ];
    buildDepends = with self; [ dune ocaml ];
  };
  dune = {
    name = "dune";
  ...
  earlybird = {
    ...
    version = "1.1.0";
    src = pkgs.fetchurl {
      url = "https://github.com/hackwaly/ocamlearlybird/releases/download/1.1.0/earlybird-1.1.0.tbz";
      sha256 = "aae7257fa73a502ea808eb5c3f3bf9fa0218cedf43342289ca44f03b32839fdf";
    };
    opam = "${repo}/packages/earlybird/earlybird.1.1.0/opam";
    depends = with self; [ cmdliner csexp dap dune fmt iter logs lru lwt
                           lwt_ppx lwt_react menhirLib ocaml
                           ocaml-compiler-libs path_glob ppx_deriving
                           ppx_deriving_yojson sexplib ];
  ...
  path_glob = {
    ...
    version = "0.2";
    src = pkgs.fetchurl {
      url = "https://gasche.gitlab.io/path_glob/releases/path_glob-0.2.tbz";
      sha256 = "5e09a2148876b68ac8fb315679ba69b1e207ced55d91a3ea5b3046f917102a07";
    };
    opam = "${repo}/packages/path_glob/path_glob.0.2/opam";
    depends = with self; [ dune ocaml ];
    buildDepends = with self; [ dune ocaml ];
  };
  ppx_assert = {
