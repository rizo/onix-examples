...
} }:
{
  ...
  ordma = {
    ...
    version = "0.0.5";
    src = pkgs.fetchurl {
      url = "https://github.com/toolslive/ordma/archive/0.0.5.tar.gz";
      sha256 = "0a8qnciak9mw4jgw12ncqni66kyx17a0abznmvhbhri5pj4an0k7";
    };
    opam = "${repo}/packages/ordma/ordma.0.0.5/opam";
    depends = with self; [ dune lwt lwt_log ocaml ];
    buildDepends = with self; [ dune ocaml ];
    depexts = with pkgs; [ (pkgs.libibverbs-dev or null)
                           (pkgs.libibverbs-devel or null)
                           (pkgs.librdmacm-dev or null)
                           (pkgs.librdmacm-devel or null) ];
  };
  result = {
    name = "result";
    version = "1.5";
