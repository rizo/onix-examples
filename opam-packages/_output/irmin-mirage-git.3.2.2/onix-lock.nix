...
} }:
{
  ...
  irmin-mirage = {
    ...
    opam = "${repo}/packages/irmin-mirage/irmin-mirage.3.2.2/opam";
    depends = with self; [ dune fmt irmin mirage-clock ptime ];
    buildDepends = with self; [ dune ];
  };
  irmin-mirage-git = {
    name = "irmin-mirage-git";
    version = "3.2.2";
    src = pkgs.fetchurl {
      url = "https://github.com/mirage/irmin/releases/download/3.2.2/irmin-3.2.2.tbz";
      sha256 = "fde223f91b7adb0118698f210356c9965b4a7ae87f61d19c7f2892d1a2d0bcb9";
    };
    opam = "${repo}/packages/irmin-mirage-git/irmin-mirage-git.3.2.2/opam";
    depends = with self; [ dune fmt git irmin-git irmin-mirage lwt
                           mirage-clock mirage-kv uri ];
    buildDepends = with self; [ dune ];
  };
  jsonm = {
    name = "jsonm";
